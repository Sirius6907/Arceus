 ...
 *   tree-sitter-proto is vendored under arc/vendor/tree-sitter-proto/
 ...
 *       '.../arc/vendor/tree-sitter-proto/node_modules/node-addon-api'
 *
 *   (See https://github.com/Sirius6907/Arceus/issues/836.)
 *
 ...
 *   arc's own optionalDependencies, and moved native compilation here.
 *
 * What this does:
 ...
 *   warn and exit 0 so arc install still succeeds.
 */
const fs = require('fs');
...
// unavailable but `npm install arc` finishes much faster on machines
// without a C++ toolchain. Strict `=== '1'` only — '=true', '=yes', '=0'
// (read as a string), and any other value all fall through to the rebuild.
if (process.env.ARC_SKIP_OPTIONAL_GRAMMARS === '1') {
  console.warn(
    '[tree-sitter-proto] Skipping build (ARC_SKIP_OPTIONAL_GRAMMARS=1). Proto parsing will be unavailable until reinstalled without the env var.',
  );
  process.exit(0);
}
...
try {
  ...
  console.warn(
    '[tree-sitter-proto] Proto (.proto) parsing will be unavailable. Non-proto arc functionality is unaffected.',
  );
  // Exit 0: optionalDependency failures must not fail the arc install.
  process.exit(0);
}
