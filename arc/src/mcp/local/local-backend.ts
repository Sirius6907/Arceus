...
export class LocalBackend {
  ...
  private async refreshRepos(): Promise<void> {
    ...
    for (const entry of entries) {
      ...
      if (kuzu.found && kuzu.needsReindex) {
        logger.error(
          `Arceus: "${entry.name}" has a stale KuzuDB index. Run: arc analyze ${entry.path}`,
        );
      }
  ...
  private async bm25Search(
    ...
    try {
      ...
      logger.warn(
        { err: err?.message },
        'Arceus: bm25-index.js import failed — falling back to semantic-only',
      );
    ...
    try {
      ...
      logger.error(
        { err: err.message },
        'Arceus: BM25/FTS search failed (FTS indexes may not exist) -',
      );
      return { results: [], ftsUsed: false };
    }
  ...
  private async semanticSearch(repo: RepoHandle, query: string, limit: number): Promise<any[]> {
    try {
      ...
      if (isVectorExtensionSupportedByPlatform()) {
        ...
        logger.warn(
          'Arceus [query:vector]: VECTOR extension not supported on this platform; using exact scan fallback',
        );
      }
