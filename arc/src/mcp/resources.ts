 ...
 * All resources use repo-scoped URIs: arc://repo/{name}/context
 */
...
export function getResourceTemplates(): ResourceTemplate[] {
  return [
    {
      uriTemplate: 'arc://repo/{name}/context',
      name: 'Repo Overview',
      description: 'Codebase stats, staleness check, and available tools',
      mimeType: 'text/yaml',
    },
    {
      uriTemplate: 'arc://repo/{name}/clusters',
      name: 'Repo Modules',
      description: 'All functional areas (Leiden clusters)',
      mimeType: 'text/yaml',
    },
    {
      uriTemplate: 'arc://repo/{name}/processes',
      name: 'Repo Processes',
      description: 'All execution flows',
      mimeType: 'text/yaml',
    },
    {
      uriTemplate: 'arc://repo/{name}/schema',
      name: 'Graph Schema',
      description: 'Node/edge schema for Cypher queries',
      mimeType: 'text/yaml',
    },
    {
      uriTemplate: 'arc://repo/{name}/cluster/{clusterName}',
      name: 'Module Detail',
      description: 'Deep dive into a specific functional area',
      mimeType: 'text/yaml',
    },
    {
      uriTemplate: 'arc://repo/{name}/process/{processName}',
      name: 'Process Trace',
      description: 'Step-by-step execution trace',
      mimeType: 'text/yaml',
    },
    {
      uriTemplate: 'arc://group/{name}/contracts',
    ...
    {
      uriTemplate: 'arc://group/{name}/status',
...
/** Query parameters for `arc://group/{name}/contracts` */
...
/** Normalized parse result for Arceus MCP resource URIs */
export type ParsedArceusResource =
  | { kind: 'repos' }
  | { kind: 'setup' }
  | {
      kind: 'repo';
...
/**
 * Parse a Arceus resource URI (repos, setup, per-repo, or per-group templates).
 * Used by `readResource` and tests (round-trip / dispatch coverage).
 */
export function parseResourceUri(uri: string): ParsedArceusResource {
  if (uri === 'arc://repos') return { kind: 'repos' };
  if (uri === 'arc://setup') return { kind: 'setup' };
  ...
  if (u.hostname === 'group') {
    ...
    if (segments.length < 2) {
      throw new Error(
        `Invalid group resource URI (expected arc://group/{name}/contracts or .../status): ${uri}`,
      );
    }
...
async function getReposResource(backend: LocalBackend): Promise<string> {
  ...
  if (repos.length === 0) {
    return 'repos: []\n# No repositories indexed. Run: arc analyze';
  }
  ...
  if (repos.length > 1) {
    ...
    lines.push(`# arc_search({query: "auth", repo: "${repos[0].name}"})`);
  }
...
async function getContextResource(backend: LocalBackend, repoName?: string): Promise<string> {
  ...
  if (!context) {
    return 'error: No codebase loaded. Run: arc analyze';
  }
  ...
  lines.push('');
  lines.push('re_index: Run `npx arc analyze` in terminal if data is stale');
  lines.push('');
  lines.push('resources_available:');
  lines.push('  - arc://repos: All indexed repositories');
  lines.push(`  - arc://repo/${context.projectName}/clusters: All functional areas`);
  lines.push(`  - arc://repo/${context.projectName}/processes: All execution flows`);
  lines.push(`  - arc://repo/${context.projectName}/cluster/{name}: Module details`);
  lines.push(`  - arc://repo/${context.projectName}/process/{name}: Process trace`);
  lines.push(
    '  - arc://group/{name}/contracts: Group contract registry (optional ?type=&repo=&unmatchedOnly=)',
  );
  lines.push('  - arc://group/{name}/status: Group index / contract staleness');
...
async function getClustersResource(backend: LocalBackend, repoName?: string): Promise<string> {
  try {
    ...
    if (!result.clusters || result.clusters.length === 0) {
      return 'modules: []\n# No functional areas detected. Run: arc analyze';
    }
    ...
    if (result.clusters.length > displayLimit) {
      lines.push(
        `\n# Showing top ${displayLimit} of ${result.clusters.length} modules. Use arc_query for deeper search.`,
      );
    }
...
async function getProcessesResource(backend: LocalBackend, repoName?: string): Promise<string> {
  try {
    ...
    if (!result.processes || result.processes.length === 0) {
      return 'processes: []\n# No processes detected. Run: arc analyze';
    }
    ...
    if (result.processes.length > displayLimit) {
      lines.push(
        `\n# Showing top ${displayLimit} of ${result.processes.length} processes. Use arc_query for deeper search.`,
      );
    }
 ...
 * Useful for `arc setup` onboarding or dynamic content injection.
 */
async function getSetupResource(backend: LocalBackend): Promise<string> {
  const repos = await backend.listRepos();

  if (repos.length === 0) {
    return '# Arceus\n\nNo repositories indexed. Run: `npx arc analyze` in a repository.';
  }
  ...
  for (const repo of repos) {
    ...
    const lines = [
      `# Arceus MCP — ${repo.name}`,
      '',
      `This project is indexed by Arceus as **${repo.name}** (${stats.nodes || 0} symbols, ${stats.edges || 0} relationships, ${stats.processes || 0} execution flows).`,
      '',
      '## Tools',
      '',
      ...
      '## Resources',
      '',
      `- \`arc://repo/${repo.name}/context\` — Stats, staleness check`,
      `- \`arc://repo/${repo.name}/clusters\` — All functional areas`,
      `- \`arc://repo/${repo.name}/processes\` — All execution flows`,
      `- \`arc://repo/${repo.name}/schema\` — Graph schema for Cypher`,
    ];
