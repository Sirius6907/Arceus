/**
 * `arc publish` — opt-in ping to the understand-quickly registry.
 *
...
export const publishCommand = async (
  ...
  // run `arc analyze` yet but want to verify the command is wired.
  ...
  // ── 2. Confirm a Arceus index exists ───────────────────────────────
  ...
  if (!(await hasIndex(repoPath))) {
    cliError(
      `[understand-quickly] no Arceus index found at ${repoPath}/.arc.\n` +
        'Run `arc analyze` first, then re-run `arc publish`.',
    );
    process.exitCode = 1;
    return;
  }
  ...
  try {
    ...
    // arc/src/core/embeddings/http-client.ts so the user sees the
