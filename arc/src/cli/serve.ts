...
process.on('uncaughtException', (err) => {
  logger.error({ err }, '[arc serve] Uncaught exception');
  flushLoggerSync();
  process.exit(1);
});
process.on('unhandledRejection', (reason) => {
  const err = reason instanceof Error ? reason : new Error(String(reason));
  logger.error({ err }, '[arc serve] Unhandled rejection');
  flushLoggerSync();
  process.exit(1);
});
...
export const serveCommand = async (options?: { port?: string; host?: string }) => {
  ...
  try {
    ...
    if (err.code === 'EADDRINUSE') {
      cliError(
        `\nFailed to start Arceus server:\n` +
          `  ${err.message || err}\n\n` +
          `  Port ${port} is already in use. Either:\n` +
          `    1. Stop the other process using port ${port}\n` +
          `    2. Use a different port: arc serve --port 4748\n`,
