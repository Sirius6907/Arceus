 ...
 *
 * Exposes Arceus tools (query, context, impact, cypher) as direct CLI commands.
 * Bypasses MCP entirely — invokes LocalBackend directly for minimal overhead.
 *
 * Usage:
 *   arc query "authentication flow"
 *   arc context --name "validateUser"
 *   arc impact --target "AuthService" --direction upstream
...
async function getBackend(): Promise<LocalBackend> {
  ...
  if (!ok) {
    cliError(`Arceus: No indexed repositories found. Run: ${binName} analyze`);
    process.exit(1);
  }
  return _backend;
}
...
function output(data: any): void {
  ...
  try {
    ...
    if (err?.code === 'EPIPE') {
      // Consumer closed the pipe (e.g., `arc cypher ... | head -1`)
...
export async function queryCommand(
  ...
  if (!queryText?.trim()) {
    cliError(`Usage: ${binName} query <search_query>`);
    process.exit(1);
  }
...
export async function contextCommand(
  ...
  if (!name?.trim() && !options?.uid) {
    cliError(`Usage: ${binName} context <symbol_name> [--uid <uid>] [--file <path>]`);
    process.exit(1);
  }
...
export async function impactCommand(
  ...
  if (!target?.trim()) {
    cliError(`Usage: ${binName} impact <symbol_name> [--direction upstream|downstream]`);
    process.exit(1);
  }
  ...
  try {
    ...
    output({
      ...
      target: { name: target },
      direction: options?.direction || 'upstream',
      suggestion: `Try reducing --depth or using ${binName} context <symbol> as a fallback`,
    });
    process.exit(1);
  }
}
...
export async function cypherCommand(
  ...
  if (!query?.trim()) {
    cliError(`Usage: ${binName} cypher <cypher_query>`);
    process.exit(1);
  }
