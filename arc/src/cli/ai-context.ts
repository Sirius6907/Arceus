 ...
 *
 * Creates AGENTS.md and CLAUDE.md with full inline Arceus context.
...
/**
 * Generate the full Arceus context content.
 *
...
async function findGroupsContainingRegistryName(registryName: string): Promise<string[]> {
  ...
  for (const g of names) {
    try {
      const config = await loadGroupConfig(getGroupDir(getDefaultArceusDir(), g));
...
function generateArceusContent(
  projectName: string,
  stats: RepoStats,
...
${tableBody}`
  ...
  return `${ARC_START_MARKER}
# Arceus — Code Intelligence

This project is indexed by Arceus as **${projectName}**${noStats ? '' : ` (${stats.nodes || 0} symbols, ${stats.edges || 0} relationships, ${stats.processes || 0} execution flows)`}. Use the Arceus MCP tools to understand code, assess impact, and navigate safely.
...
| Resource | Use for |
|----------|---------|
| \`arc://repo/${projectName}/context\` | Codebase overview, check index freshness |
| \`arc://repo/${projectName}/clusters\` | All functional areas |
| \`arc://repo/${projectName}/processes\` | All execution flows |
| \`arc://repo/${projectName}/process/{name}\` | Step-by-step execution trace |

${
  groupNames && groupNames.length > 0
    ? `## Cross-Repo Groups

This repository is listed under Arceus **group(s): ${groupNames.join(', ')}** (see \`~/.arc/groups/\`). For cross-repo analysis, use MCP tools \`impact\`, \`query\`, and \`context\` with \`repo\` set to \`@<groupName>\` or \`@<groupName>/<memberPath>\` (paths match keys in that group’s \`group.yaml\`). Use \`group_list\` / \`group_sync\` for membership and sync. From the terminal: \`npx arc group list\`, \`npx arc group sync <name>\`, \`npx arc group impact <name> --target <symbol> --repo <group-path>\`.
...
/**
 * Create or update Arceus section in a file
 * - If file doesn't exist: create with Arceus content
 * - If file exists without Arceus section: append
 * - If file exists with Arceus section: replace that section
 */
async function upsertArceusSection(
  filePath: string,
  content: string,
  ...
  // Check if Arceus section already exists. Matching is restricted
  // to markers that occupy their own line so that inline prose
  // references (e.g. `` See the `<!-- arc:start -->` block `` in
  ...
  const startIdx = findSectionMarkerIndex(existingContent, ARC_START_MARKER);
  ...
  if (startIdx !== -1 && endIdx !== -1 && endIdx > startIdx) {
    ...
    const after = existingContent.substring(endIdx + ARC_END_MARKER.length);
...
/**
 * Install Arceus skills to .claude/skills/arc/
 * Works natively with Claude Code, Cursor, and GitHub Copilot
 */
async function installSkills(repoPath: string): Promise<string[]> {
  const skillsDir = path.join(repoPath, '.claude', 'skills', 'arc');
  ...
  const skills = [
    ...
    {
      name: 'arc-impact-analysis',
      description:
        'Use when the user wants to know what will break if they change something, or needs safety analysis before editing code. Examples: "Is it safe to change X?", "What depends on this?", "What will break?"',
    },
    {
      name: 'arc-refactoring',
      description:
    ...
    },
    {
      name: 'arc-guide',
      description:
        'Use when the user asks about Arceus itself — available tools, how to query the knowledge graph, MCP resources, graph schema, or workflow reference. Examples: "What Arceus tools are available?", "How do I use Arceus?"',
    },
    {
      name: 'arc-cli',
      description:
        'Use when the user needs to run Arceus CLI commands like analyze/index a repo, check status, clean the index, generate a wiki, or list indexed repos. Examples: "Index this repo", "Reanalyze the codebase", "Generate a wiki"',
    },
  ];
...
Use Arceus tools to accomplish this task.
`;
      }
...
export async function generateAIContextFiles(
  ...
  const content = generateArceusContent(
    projectName,
    stats,
    generatedSkills,
    groupNames,
  ...
  if (!options?.skipAgentsMd) {
    ...
    const agentsResult = await upsertArceusSection(agentsPath, content);
    createdFiles.push(`AGENTS.md (${agentsResult})`);

    // Create CLAUDE.md (for Claude Code)
    const claudePath = path.join(repoPath, 'CLAUDE.md');
    const claudeResult = await upsertArceusSection(claudePath, content);
  ...
  // Install skills to .claude/skills/arc/ (unless --skip-skills)
  if (!options?.skipSkills) {
    const installedSkills = await installSkills(repoPath);
    if (installedSkills.length > 0) {
      createdFiles.push(`.claude/skills/arc/ (${installedSkills.length} skills)`);
    }
  } else {
    createdFiles.push('.claude/skills/arc/ (skipped via --skip-skills)');
  }
