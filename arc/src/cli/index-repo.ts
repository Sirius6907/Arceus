 ...
 * Index Command
 *
 * Registers an existing .arc/ folder into the global registry so the
 * MCP server can discover the repo without running a full `arc analyze`.
 *
 * Useful when a pre-built .arc/ directory is already present (e.g. after
...
export const indexCommand = async (inputPathParts?: string[], options?: IndexOptions) => {
  ...
  if (!options?.allowNonGit && !isGitRepo(repoPath)) {
    ...
    console.log('  Or use --allow-non-git to register an existing .arc index anyway.\n');
    process.exitCode = 1;
    return;
  }
  ...
  try {
    ...
    console.log(`  No .arc/ folder found at: ${storagePath}`);
    console.log('  Run `arc analyze` to build the index first.\n');
    process.exitCode = 1;
    return;
  }
  ...
  try {
    ...
    console.log(`  .arc/ folder exists but contains no LadybugDB index.`);
    console.log('  Run `arc analyze` to build the index.\n');
    process.exitCode = 1;
    return;
  }
  ...
  if (!meta) {
    if (!options?.force) {
      console.log(`  .arc/ exists but meta.json is missing.`);
      console.log('  Use --force to register anyway (stats will be empty),');
      console.log('  or run `arc analyze` to rebuild properly.\n');
      process.exitCode = 1;
      return;
    }
  ...
  // it's missing, so an `index` of an older `.arc/` still gets
