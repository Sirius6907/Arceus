 ...
 * `arc/test/integration/mcp/import-closure.test.ts` enforces this.
 */
...
export const mcpCommand = async () => {
  ...
  // startMCPServer (arc/src/mcp/server.ts) so the server's shutdown
  ...
  // `arc analyze` already warns at index time, filtered by the repo's
  ...
  if (repos.length === 0) {
    ...
    logger.warn(
      'Arceus: No indexed repos yet. Run `arc analyze` in a git repo — the server will pick it up automatically.',
    );
  } else {
    logger.info(
      { repoCount: repos.length, repos: repos.map((r) => r.name) },
      'Arceus: MCP server starting',
    );
  }
