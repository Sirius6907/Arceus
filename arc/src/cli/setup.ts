 ...
 * so the Arceus MCP server is available in all projects.
 */
...
// `arc@latest` since they're quickstart docs, not persisted state.
const _require = createRequire(import.meta.url);
const _pkg = _require('../../package.json') as { version?: unknown };
if (typeof _pkg.version !== 'string' || !_pkg.version) {
  throw new Error(
    'arc/package.json#version is missing or not a string — cannot generate MCP fallback config.',
  );
}
const NPX_REF = `arc@${_pkg.version}`;
...
/**
 * Resolve the absolute path to the `arc` binary if it's installed
 * globally (or via npm -g / yarn global). Returns null when not found.
 */
function resolveArceusBin(): string | null {
  try {
    const isWin = process.platform === 'win32';
    const cmd = isWin ? 'where' : 'which';
    const output = execFileSync(cmd, ['arc'], {
      encoding: 'utf-8',
      timeout: 5000,
 ...
 *
 * Prefers the globally-installed `arc` binary (starts in ~1 s) over
 * `npx -y arc@<version>` (cold-cache install of native deps can take
 * >60 s, exceeding Claude Code's 30 s MCP connection timeout). The fallback
 * version is read from arc/package.json#version at module load so the
 * persisted user config matches the installed package.
 *
 * Falls back to npx when the binary isn't on PATH — e.g. first-time
 * users who ran `npx arc analyze` but haven't done `npm i -g`.
 */
function getMcpEntry() {
  const bin = resolveArceusBin();
...
async function setupCursor(result: SetupResult): Promise<void> {
  ...
  try {
    const ok = await mergeJsoncFile(mcpPath, ['mcpServers', 'arc'], getMcpEntry());
    if (ok) {
...
/**
 * Install Arceus skills to ~/.claude/skills/ for Claude Code.
 */
...
/**
 * Check whether an event array already contains a arc-hook entry.
 */
function hasArceusHook(hooksObj: any, eventName: string): boolean {
...
/**
 * Install Arceus hooks to ~/.claude/settings.json for Claude Code.
...
async function installClaudeCodeHooks(result: SetupResult): Promise<void> {
  ...
  // Source hooks bundled within the arc package (hooks/claude/)
  const pluginHooksPath = path.join(__dirname, '..', '..', 'hooks', 'claude');

  // Copy unified hook script to ~/.claude/hooks/arc/
  const destHooksDir = path.join(claudeDir, 'hooks', 'arc');

  try {
    await fs.mkdir(destHooksDir, { recursive: true });

    const src = path.join(pluginHooksPath, 'arc-hook.cjs');
    const dest = path.join(destHooksDir, 'arc-hook.cjs');
    try {
    ...
    const hookPath = path.join(destHooksDir, 'arc-hook.cjs').replace(/\\/g, '/');
    ...
    if (!hasArceusHook(parsed?.hooks, 'PreToolUse')) {
      hookEntries.push({
          ...
          hooks: [
            {
              ...
              command: hookCmd,
              timeout: 10,
              statusMessage: 'Enriching with Arceus graph context...',
            },
          ],
        },
      });
    }
    if (!hasArceusHook(parsed?.hooks, 'PostToolUse')) {
      hookEntries.push({
          ...
          hooks: [
            {
              ...
              command: hookCmd,
              timeout: 10,
              statusMessage: 'Checking Arceus index freshness...',
            },
          ],
        },
      });
...
async function setupOpenCode(result: SetupResult): Promise<void> {
  ...
  try {
    const ok = await mergeJsoncFile(configPath, ['mcp', 'arc'], getOpenCodeMcpEntry());
    if (ok) {
...
/**
 * Append Arceus MCP server config to Codex's config.toml if missing.
 */
async function upsertCodexConfigToml(configPath: string): Promise<void> {
  ...
  if (existing.includes('[mcp_servers.arc]')) {
    return;
  }
...
async function setupCodex(result: SetupResult): Promise<void> {
  ...
  try {
    const entry = getMcpEntry();
    await execFileAsync('codex', ['mcp', 'add', 'arc', '--', entry.command, ...entry.args], {
...
/**
 * Install Arceus skills to a target directory.
 * Each skill is installed as {targetDir}/arc-{skillName}/SKILL.md
...
/**
 * Install global Cursor skills to ~/.cursor/skills/arc/
 */
...
/**
 * Install global OpenCode skills to ~/.config/opencode/skills/arc/
 */
...
/**
 * Install global Codex skills to ~/.agents/skills/arc/
 */
