 ...
 * via ARC_SKIP_OPTIONAL_GRAMMARS=1 (postinstall scripts), or it can
 ...
 * those who installed with `ARC_SKIP_OPTIONAL_GRAMMARS=1` or whose
  ...
  for (const g of OPTIONAL_GRAMMARS) {
    try {
      ...
      if (!looksMissing) {
        ...
        cliWarn(
          `Arceus: optional grammar "${g.name}" is installed but failed to load (${msg.slice(0, 200)}). ${g.extensions.join('/')} files will not be parsed.`,
...
export function warnMissingOptionalGrammars(opts?: {
  ...
  for (const g of missing) {
    ...
    cliWarn(
      `Arceus${ctx}: optional grammar "${g.name}" is unavailable — ${g.extensions.join('/')} files will not be parsed. Reinstall without ARC_SKIP_OPTIONAL_GRAMMARS=1 (and ensure python3, make, g++) to enable.`,
