...
export const wikiCommand = async (inputPath?: string, options?: WikiCommandOptions) => {
  ...
  if (options?.verbose) {
    process.env.ARC_VERBOSE = '1';
  }

  console.log('\n  Arceus Wiki Generator\n');
  ...
  if (!meta) {
    console.log('  Error: No Arceus index found.');
    console.log('  Run `arc analyze` first to index this repository.\n');
    process.exitCode = 1;
    return;
  }
  ...
  if (
    ...
    console.log('  Config saved to ~/.arc/config.json\n');
  }
  ...
  if (!hasSavedConfig && !hasCLIOverrides) {
    if (!process.stdin.isTTY) {
      ...
      if (!llmConfig.apiKey && llmConfig.provider !== 'cursor') {
        ...
        console.log('  Set OPENAI_API_KEY or ARC_API_KEY environment variable,');
      ...
      if (choice === '5' && hasCursor) {
        ...
        const envKey = process.env.ARC_API_KEY || process.env.OPENAI_API_KEY || '';
        let azureKey: string;
        if (envKey) {
  ...
  try {
    ...
    if (options?.review && result.moduleTree) {
      ...
      if (choice === 'n' || choice === 'no') {
        console.log('\n  Generation cancelled. Run `arc wiki` later to generate.\n');
        return;
      }
      ...
      if (choice === 'edit' || choice === 'e') {
        ...
        try {
          ...
          console.log('  Then run `arc wiki` to continue.\n');
          return;
        }
      }
    ...
    if (err.message?.includes('No source files')) {
      ...
      if (isAuthError && process.stdin.isTTY) {
        ...
        if (!answer || answer === 'y' || answer === 'yes') {
          ...
          await saveCLIConfig({});
          console.log('  Config cleared. Run `arc wiki` again to reconfigure.\n');
        }
      }
    } else {
      console.log(`\n  Error: ${err.message}\n`);
      if (process.env.ARC_VERBOSE) {
...
function publishGist(htmlPath: string): { url: string; rawUrl: string } | null {
  try {
    const output = execFileSync(
      'gh',
      ['gist', 'create', htmlPath, '--desc', 'Repository Wiki — generated by Arceus', '--public'],
