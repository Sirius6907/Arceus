 ...
 *
 * Usage:
 *   arc eval-server                    # default port 4848
 *   arc eval-server --port 4848        # explicit port
 *   arc eval-server --idle-timeout 300 # auto-shutdown after 300s idle
 *
 * API:
...
export function formatContextResult(result: any): string {
  ...
  if (result.status === 'ambiguous') {
    ...
    lines.push(`\nRe-run: arc-context "${result.candidates?.[0]?.name}" "<file_path>"`);
    return lines.join('\n');
  }
...
function getNextStepHint(toolName: string): string {
  switch (toolName) {
    case 'query':
      return '\n---\nNext: Pick a symbol above and run arc-context "<name>" to see all its callers, callees, and execution flows.';

    case 'context':
      return '\n---\nNext: To check what breaks if you change this, run arc-impact "<name>" upstream';

    case 'impact':
      return '\n---\nNext: Review d=1 items first (WILL BREAK). Read the source with cat to understand the code, then make your fix.';

    case 'cypher':
      return '\n---\nNext: To explore a result symbol in depth, run arc-context "<name>"';

    case 'detect_changes':
      return '\n---\nNext: Run arc-context "<symbol>" on high-risk changed symbols to check their callers.';
...
export async function evalServerCommand(options?: EvalServerOptions): Promise<void> {
  ...
  if (!ok) {
    ...
    cliWarn('Arceus eval-server: No indexed repositories found. Run: arc analyze');
    process.exit(1);
  }
  ...
  logger.info(
    ...
    'Arceus eval-server: repos loaded',
  );
  ...
  function resetIdleTimer() {
    ...
    idleTimer = setTimeout(async () => {
      logger.info({ idleTimeoutSec }, 'Arceus eval-server: idle timeout reached, shutting down');
  ...
  server.listen(port, '127.0.0.1', () => {
    ...
    const bannerLines = [
      `Arceus eval-server: listening on http://127.0.0.1:${port}`,
    ...
    try {
      ...
      writeSync(1, `ARC_EVAL_SERVER_READY:${port}\n`);
    } catch {
  ...
  const shutdown = async () => {
    logger.info('Arceus eval-server: shutting down...');
    await backend.disconnect();
    server.close();
    process.exit(0);
  };
