...
export const statusCommand = async () => {
  ...
  if (!repo) {
    ...
    if (await hasKuzuIndex(storagePath)) {
      ...
      console.log('Run: arc analyze   (rebuilds the index with LadybugDB)');
    } else {
      console.log('Repository not indexed.');
      console.log('Run: arc analyze');
    }
    return;
  }
  ...
  console.log(`Status: ${isUpToDate ? '✅ up-to-date' : '⚠️ stale (re-run arc analyze)'}`);
};
