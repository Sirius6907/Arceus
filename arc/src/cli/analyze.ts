 ...
 * Analyze Command
 *
 * Indexes a repository and stores the knowledge graph in .arc/
 *
...
export interface AnalyzeOptions {
  ...
  skills?: boolean;
  verbose?: boolean;
  /** Skip AGENTS.md and CLAUDE.md arc block updates. */
  skipAgentsMd?: boolean;
  /** Omit volatile symbol/relationship counts from AGENTS.md and CLAUDE.md. */
  noStats?: boolean;
  /** Skip installing standard Arceus skill files to .claude/skills/arc/. */
  skipSkills?: boolean;
   ...
   * `ARC_MAX_FILE_SIZE` for the rest of the pipeline.
   */
  maxFileSize?: string;
...
export const analyzeCommand = async (inputPath?: string, options?: AnalyzeOptions) => {
  ...
  if (options?.maxFileSize) {
    process.env.ARC_MAX_FILE_SIZE = options.maxFileSize;
  }
  ...
  if (options?.embeddingDevice) {
    ...
    process.env.ARC_EMBEDDING_DEVICE = options.embeddingDevice;
  }
  ...
  // See `arc/src/core/logger.ts:flushLoggerSync`.
  let aborted = false;
  const sigintHandler = () => {
    ...
    if (
      ...
      cliError(
        `  This error typically occurs on very large repositories.\n` +
          `  Suggestions:\n` +
          `    1. Add large vendored/generated directories to .arcignore\n` +
      ...
      // 'node.target'" crash happens inside npm *before* arc code runs,
