 ...
 * Remove Command (#664)
 *
 * Delete the `.arc/` index for a registered repo and unregister it
 * from the global registry (~/.arc/registry.json). The target is
...
export const removeCommand = async (target: string, options?: { force?: boolean }) => {
  ...
  if (!options?.force) {
    console.log(`This will delete the Arceus index for: ${entry.name}`);
  ...
  // `<entry.path>/.arc` subfolder. `~/.arc/registry.json` is
