 ...
 * Git Clone Utility
 *
 * Shallow-clones repositories into ~/.arc/repos/{name}/.
...
const CLONE_ROOT = path.resolve(path.join(os.homedir(), '.arc', 'repos'));
 ...
 * collide with an existing `~/.arc/repos/repo` cloned from a different
 ...
 *
 * Security:
 *   - targetDir must resolve inside CLONE_ROOT (~/.arc/repos/). The
