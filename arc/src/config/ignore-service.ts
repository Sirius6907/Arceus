...
// hardcoded rule via a `!pattern` line in `.arcignore` (#771) — same
...
/**
 * Load .gitignore and .arcignore rules from the repo root.
 * Returns an `ignore` instance with all patterns, or null if no files found.
 */
export interface IgnoreOptions {
  /** Skip .gitignore parsing, only read .arcignore. Defaults to ARC_NO_GITIGNORE env var. */
  noGitignore?: boolean;
}
...
export const loadIgnoreRules = async (
  ...
  const skipGitignore = options?.noGitignore ?? !!process.env.ARC_NO_GITIGNORE;
  const filenames = skipGitignore ? ['.arcignore'] : ['.gitignore', '.arcignore'];
...
/**
 * Walk ancestor segments of `rel` and check whether `.arcignore`
 ...
 * the user has an explicit `!__tests__/` line in `.arcignore`.
 ...
 * - .gitignore / .arcignore patterns (via `ignore` package)
 ...
 *
 * Precedence (#771): user's `.arcignore` negation patterns take
...
export const createIgnoreFilter = async (repoPath: string, options?: IgnoreOptions) => {
  ...
  return {
    ignored(p: Path): boolean {
      ...
      if (!rel) return false;
      // User's .arcignore negation takes precedence over hardcoded
      ...
      // Check .gitignore / .arcignore patterns
    ...
    childrenIgnored(p: Path): boolean {
      ...
      const rel = p.relative();
      // User's .arcignore negation takes precedence (#771) — if the
      ...
      // Check against .gitignore / .arcignore patterns.
