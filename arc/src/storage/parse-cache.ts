 ...
 *   - The current `arc` npm package version, read at module load.
 ...
 *     running `npm i -g arc@latest` after a parser-affecting
 ...
 */
const SCHEMA_BUMP = 1;
const ARC_PKG_VERSION = (() => {
  try {
    // package.json sits at arc/package.json — two levels up from
    // arc/src/storage/parse-cache.ts (or its dist/ equivalent).
    const here = path.dirname(fileURLToPath(import.meta.url));
    const candidates = [
      path.join(here, '..', '..', 'package.json'), // src/storage → arc/
      path.join(here, '..', '..', '..', 'package.json'), // dist/storage → arc/
    ];
...
export const PARSE_CACHE_VERSION = `${SCHEMA_BUMP}+${ARC_PKG_VERSION}`;
