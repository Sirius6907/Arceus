...
import { NODE_TABLES, REL_TABLE_NAME, REL_TYPES, EMBEDDING_TABLE_NAME } from 'arceus-shared';
...
export type { NodeTableName, RelType } from 'arceus-shared';
...
const _rawDims = parseInt(process.env.ARC_EMBEDDING_DIMS ?? '384', 10);
if (Number.isNaN(_rawDims) || _rawDims <= 0) {
  throw new Error(
    `ARC_EMBEDDING_DIMS must be a positive integer, got "${process.env.ARC_EMBEDDING_DIMS}"`,
  );
}
