 ...
 * pinned explicitly by every caller, otherwise Arceus regresses:
 *
...
/**
 * Upper bound for any single Arceus LadybugDB file (graph index, group
 * bridge, install scratch, test fixture). 16 GiB is intentionally generous
 * for real-world code graphs (the Arceus self-index uses < 50 MiB) while
 * remaining far below any 64-bit OS mmap ceiling.
 *
 * Override with the `ARC_LBUG_MAX_DB_SIZE` environment variable when
...
export const LBUG_MAX_DB_SIZE: number = (() => {
  const raw = process.env.ARC_LBUG_MAX_DB_SIZE;
  if (raw) {
...
export const WAL_RECOVERY_SUGGESTION =
  'WAL corruption detected. Run `arc analyze` to rebuild the index.';
...
//
// All four Arceus retry pairs that touch native LadybugDB locks live with
 ...
 * `createTempDir` in `arc/test/helpers/test-db.ts` and the prefixes
 * used by `withTestLbugDB` (`arc/test/helpers/test-indexed-db.ts`).
 ...
 *
 * The default `createTempDir('arc-test-')` and the lbug variant
 * `'arc-lbug-'` cover today's call sites.
 */
const TEST_FIXTURE_PREFIXES = ['arc-lbug-', 'arc-test-'];
 ...
 *
 * The symbol is internal to Arceus; consumers should treat the underlying
 * error message as the user-visible signal.
 */
export const LBUG_OPEN_RETRY_EXHAUSTED = Symbol.for('arc.lbug.openRetryExhausted');
 ...
 *     `<tmp>/arc-lbug-x/../../etc/passwd` is rejected.
 ...
 *     with `arc-lbug-` accept arbitrary nested paths under it.
 */
