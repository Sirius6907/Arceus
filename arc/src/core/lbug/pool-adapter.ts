...
// Stdout-capture state lives in `arc/src/mcp/stdio-capture.ts` — a leaf
// module with zero non-`node:` imports. We re-export the same symbols here
// so the existing test mock seam (`arc/src/mcp/core/lbug-adapter.ts`
  ...
  try {
    ...
    throw new Error(
      `LadybugDB WAL corruption detected for ${repoId}. ` +
        `Run \`arc analyze\` to rebuild the index. (quarantine failed)`,
    );
  }
  realStderrWrite(
    `Arceus: LadybugDB WAL quarantined for ${repoId}; graph may be stale. ` +
      `Run \`arc analyze\` to rebuild the index.\n`,
  );
 ...
 * Retries on lock errors (e.g., when `arc analyze` is running).
 *
...
async function doInitLbug(repoId: string, dbPath: string): Promise<void> {
  ...
  try {
    ...
    throw new Error(`LadybugDB not found at ${dbPath}. Run: arc analyze`);
  }
  ...
  if (!shared) {
    ...
    // avoids lock conflicts when `arc analyze` is writing.
