 ...
 *     `.arc/shadow-parity/` so the static dashboard at
 *     `arc/shadow-parity-dashboard/` can render it offline.
 *
 * ## What it does NOT do
 *
 ...
 * ## Activation
 *
 * `ARC_SHADOW_MODE=1` (or `'true'`, `'yes'`, case-insensitive,
...
export interface ShadowHarness {
  /** `true` iff `ARC_SHADOW_MODE` is truthy. When `false`, `record()` is a no-op. */
...
/**
 * Construct a harness. Reads `ARC_SHADOW_MODE` at construction time
...
export function createShadowHarness(): ShadowHarness {
  const enabled = parseShadowModeEnv(process.env['ARC_SHADOW_MODE']);
...
/**
 * Env-var parser for `ARC_SHADOW_MODE`. Accepts the same truthy
