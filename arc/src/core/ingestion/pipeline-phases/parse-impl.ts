...
import { type PipelineProgress, getLanguageFromFilename } from 'arceus-shared';
 ...
 *
 * Override via ARC_CHUNK_BYTE_BUDGET (bytes) — the default of 2MB
...
const CHUNK_BYTE_BUDGET = (() => {
  const env = Number(process.env.ARC_CHUNK_BYTE_BUDGET);
...
export async function runChunkedParseAndResolve(
  ...
  parsedFiles: import('arceus-shared').ParsedFile[];
}> {
  ...
  const allParsedFiles: import('arceus-shared').ParsedFile[] = [];
  ...
  // to workers. See arc/src/storage/parse-cache.ts.
