...
const ISSUES_URL = 'https://github.com/Sirius6907/Arceus/issues';
...
const SOURCES: Record<string, GrammarSource> = {
  ...
  [SupportedLanguages.Swift]: {
    ...
    unavailableNote:
      ...
      '`arc/vendor/tree-sitter-swift`) failed to load. ' +
  ...
  [SupportedLanguages.Dart]: {
    ...
    unavailableNote:
      ...
      '`arc/vendor/tree-sitter-dart`) failed to load. ' +
...
const logFailure = (key: string, result: LoadResult): void => {
  ...
  logged.add(key);
  const message = `[arc] ${result.note} (${result.error.message})`;
