...
export function getDefaultArceusDir(): string {
  return process.env.ARC_HOME || path.join(os.homedir(), '.arc');
}

export function getGroupsBaseDir(arcDir?: string): string {
  return path.join(arcDir || getDefaultArceusDir(), 'groups');
}
...
export function getGroupDir(arcDir: string, groupName: string): string {
  validateGroupName(groupName);
  return path.join(arcDir, 'groups', groupName);
}
...
export async function listGroups(arcDir?: string): Promise<string[]> {
  const groupsDir = getGroupsBaseDir(arcDir);
  try {
...
export async function createGroupDir(
  ...
  const groupDir = getGroupDir(arcDir, groupName);
...
`;
  ...
  // user-only (no group/world bits) — arc storage is per-user
  // (`~/.arc/...`), so any "other user wants to read this" case is
