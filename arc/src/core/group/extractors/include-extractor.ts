...
export class IncludeExtractor implements ContractExtractor {
   ...
   * `arc/src/core/ingestion/filesystem-walker.ts`):
   *   - `createIgnoreFilter` honors `.gitignore`, `.arcignore`, the
   *     hardcoded ignore list, and `.arcignore` last-match-wins
   *     negation.
  ...
  private async extractProvidersGraph(
    ...
    try {
      ...
      // arc analyze stores absolute paths in the File.filePath column.
