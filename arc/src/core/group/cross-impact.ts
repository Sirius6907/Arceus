...
async function ensureBridgeReady(
  ...
  if (meta.version > 0 && meta.version !== BRIDGE_SCHEMA_VERSION) {
    return {
      error: `Bridge schema version mismatch (meta.json has ${meta.version}, expected ${BRIDGE_SCHEMA_VERSION}). Run arc group sync for this group.`,
    };
  }
  ...
  try {
    ...
    return {
      error: `No bridge.lbug in this group directory. Run arc group sync (schema ${BRIDGE_SCHEMA_VERSION}).`,
    };
  }
  const handle = await openBridgeDbReadOnly(groupDir);
  if (!handle) {
    return {
      error: `Could not open bridge.lbug read-only (schema ${BRIDGE_SCHEMA_VERSION}). Run arc group sync.`,
    };
  }
  return { handle };
}
...
export async function runGroupImpact(
  ...
  const groupDir = getGroupDir(deps.arcDir, name);
