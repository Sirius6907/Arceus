/**
 * Centralized structured logger for Arceus.
 *
 ...
 *   const childLogger = createLogger('bridge-db', { debugEnvVar: 'ARC_DEBUG_BRIDGE' });
 *
 * Operator semantics:
 ...
 *   production code logged. See `arc/test/unit/logger.test.ts` for the
 *   pattern.
 */
 ...
 *     handlers (see `arc/src/cli/serve.ts` and `arc/src/server/api.ts`).
 *
 ...
 * arc/package.json). The probe is the safety net for `--omit=optional`,
...
function isPrettyAvailable(): boolean {
  ...
  try {
    ...
    process.stderr.write(
      '[arc:logger] pino-pretty unavailable; falling back to NDJSON on stderr\n',
    );
  }
...
export function _tryBuildPrettyTransport(): LoggerOptions['transport'] | undefined {
  ...
  return {
    ...
    options: {
      ...
      // which would interleave with `arc query | jq` output.
      destination: 2,
      colorize: true,
...
function resolveBaseLevel(): string {
  const fromEnv = process.env.ARC_LOG_LEVEL;
...
/**
 * Default singleton logger (`name: 'arc'`). Backed by a Proxy so test
