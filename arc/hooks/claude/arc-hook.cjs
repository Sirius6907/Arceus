...
/**
 * Resolve the arc CLI path.
 * 1. Relative path (works when script is inside npm package)
 * 2. require.resolve (works when arc is globally installed)
...
function resolveCliPath() {
  ...
  if (!fs.existsSync(cliPath)) {
    try {
      cliPath = require.resolve('arc/dist/cli/index.js');
    } catch {
      cliPath = '';
    }
  }
 ...
 */
function runArceusCli(cliPath, args, cwd, timeout) {
...
function handlePreToolUse(input) {
  ...
  let result = '';
  try {
    const child = runArceusCli(cliPath, ['augment', '--', pattern], cwd, 7000);
