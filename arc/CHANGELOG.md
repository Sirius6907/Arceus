...
All notable changes to Arceus will be documented in this file.
...
- **`arc publish`** — opt-in command to push your indexed graph to the understand-quickly registry for shareable browsing (#1425)
...
- **Group / contracts** — `runExactMatch` honours `.arcignore` via shared `IgnoreService` (closes #1185, #1247); custom manifest links resolved against graph symbols (#1254); `IgnoreService` EACCES test under uid=0 (#1108)
...
- **CI** — fork-safe PR autofix pipeline (#1446); consolidated Claude review workflow (#1258); fine-grained PAT for RC tag push (#1407); handle expired artifacts in base coverage fetch (#1410, #1412); allow expected legacy parity failures (#1099); avoid duplicate main push checks; isolate native LadybugDB / CLI e2e flakes; seed e2e with a small fixture repo (#1249); configure e2e Arceus home at runtime; widen rate-limit test window for Windows CI (#1347)

### Changed

- **`arc publish` artefact contract** — universal opt-in publish format introduced (#1425, #1458)
...
- **Regression coverage** for `.arcignore` behaviour with `--skip-git` (#1450)
...
- `axios` (#1345), `fast-uri` 3.1.0 → 3.1.2 (#1441), `lru-cache` 11.3.5 → 11.3.6 (#1344), `mnemonist` 0.40.3 → 0.40.4 (#1239), `express-rate-limit` (#1343, #1397), `onnxruntime-node` (#1213, #1435), `uuid` 13 → 14 in /arceus-web (#1211, after revert #1222 / re-land #1250 + #1208)
...
- **`arc analyze --name <alias>`** with duplicate-name guard in the repo registry (#955)
- **`arc remove <target>`** unindexes a registered repo by name or path (#664, #1003)
...
- **Configurable large-file skip threshold** — the walker's 512 KB default is now overridable via `ARC_MAX_FILE_SIZE` (KB) or `arc analyze --max-file-size <kb>`. Values are clamped to the 32 MB tree-sitter ceiling, invalid inputs fall back to the default with a one-time warning, and the CLI banner reports the effective post-clamp threshold when an override is active (#991, #1044, #1045)
- **`ARC_INDEX_TEST_DIRS` opt-in** for `__tests__` / `__mocks__` traversal (#771, #1046)
...
- **CLI arc markers** — `<!-- arc:* -->` is only matched at section position, no longer inside code/prose (#1041, #1042)
...
- **Docker** — RC build guarded against empty `vtag`, `inputs.tag` used to detect `workflow_call` context, web builder stage now copies `arc/package.json`, base image switched from alpine to debian (#983, #996, #997, #1014)
...
- arceus-web dependency bumps: `vite` 5.4.21 → 6.4.2 → 7.3.2 → 8.0.10 + `vitest` 4 (#1061, #1062, #1063), `lucide-react` 0.562.0 → 1.11.0 with local GitHub SVG fallback (#1038), `@langchain/anthropic` 1.3.10 → 1.3.27 (#1039), `@babel/types` (#1037)
- arceus-shared dependency bumps: `typescript` (#1034)
...
- **Global-install upgrade `ENOTEMPTY`** — devendored `tree-sitter-proto` install lifecycle + preinstall cleanup so `npm i -g arc@latest` succeeds on top of an older install (#843, #846)
...
- arceus-web dependency bumps: `mermaid` 11.12.2 → 11.14.0 (#860), `tailwindcss` (#861), `jsdom` 29.0.0 → 29.0.2 (#863), `wait-on` 8.0.5 → 9.0.5 (#859), `@vitest/coverage-v8` (#864)
...
- **Runtime dependency on `file:../arceus-shared`** removed from the published package (#803)
...
- **`ARC_HOME` env var** to customize global directory (#746)
...
- **`.arcignore` negation patterns** now respected (#654)
- **MCP setup** prefers global arc binary over npx (#653)
...
### Fixed
- **`arceus-shared` module not found** — `arceus-shared` was a `file:` workspace dependency never published to npm, causing `ERR_MODULE_NOT_FOUND` when installing `arc` globally. The build now bundles shared code into `dist/_shared/` and rewrites imports to relative paths (#613)
...
### Fixed
- Incomplete fix for `arceus-shared` bundling — `prepare` script overwrote rewritten imports during publish
...
- **Unified web & CLI ingestion pipeline** — complete architectural migration of the web app from a self-contained WASM browser app to a thin client backed by the CLI server; new `arceus-shared` package for cross-package type unification (#536)
...
- **`--skip-agents-md` CLI flag** — opt out of overwriting Arceus-managed sections in AGENTS.md and CLAUDE.md during `arc analyze` (#517)
...
### Docs
- Added `arceus-shared` build step to web UI quick start instructions (#585)
- Added enterprise offering section to README (#579)

## [1.4.10] - 2026-03-27

### Fixed
- **MCP server install via npx** — resolve tree-sitter peer dependency conflicts that broke `npx -y arc@latest mcp` (#537, #538)
...
- `arc-stable-ops` added to community integrations
...
- `.gitignore` and `.arcignore` support during file discovery (#231)
