 ...
 *   - tools/list reports the Arceus tool set we expect.
 *
...
function spawnMcpServer(): SpawnedServer {
  const env: NodeJS.ProcessEnv = {
    ...
    ARC_HOME: path.join(REPO_ROOT, 'test', 'integration', 'mcp', '.tmp-home'),
    ...
    try {
      ...
      server.send({
        ...
        params: {
          ...
          capabilities: {},
          clientInfo: { name: 'arc-startup-test', version: '0.0.0' },
        },
      });
      ...
      expect(initResponse.result!.serverInfo.name).toMatch(/arc/i);
      ...
      // The published Arceus tool set. Adjust if the surface changes.
      const expectedTools = [
        'list_repos',
        'query',
        'context',
