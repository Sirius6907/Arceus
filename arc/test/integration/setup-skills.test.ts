...
describe('setupCommand skills integration', () => {
  ...
  it('reports the OpenCode skills install path with the plural skills directory', async () => {
    ...
    const installedSkill = await fs.readFile(
      path.join(tempHome, '.config', 'opencode', 'skills', 'arc-cli', 'SKILL.md'),
      'utf-8',
    );

    expect(installedSkill).toContain('Arceus CLI Commands');
    await expect(
      fs.access(path.join(tempHome, '.config', 'opencode', 'skill', 'arc-cli', 'SKILL.md')),
    ).rejects.toThrow();
  });
  ...
  it('installs packaged, flat-file, and directory skills into cursor skills directory', async () => {
    ...
    expect(skillDirs).toContain('arc-cli');

    const skillContent = await fs.readFile(
      path.join(cursorSkillsRoot, 'arc-cli', 'SKILL.md'),
      'utf-8',
    );
    expect(skillContent).toContain('Arceus CLI Commands');
  ...
  it('falls back to Codex config.toml and installs skills into ~/.agents/skills when codex CLI is unavailable', async () => {
    ...
    expect(codexConfig).toContain('[mcp_servers.arc]');
    expect(codexConfig).toMatch(/arc@\d+\.\d+\.\d+/);

    const codexSkill = await fs.readFile(
      path.join(tempHome, '.agents', 'skills', 'arc-cli', 'SKILL.md'),
      'utf-8',
    );
    expect(codexSkill).toContain('Arceus CLI Commands');
  });
  ...
  it('does not duplicate the Codex MCP section on repeated fallback setup runs', async () => {
    ...
    const codexConfig = await fs.readFile(path.join(tempHome, '.codex', 'config.toml'), 'utf-8');
    const sectionMatches = codexConfig.match(/\[mcp_servers\.arc\]/g) ?? [];
