...
beforeAll(() => {
  tmpHome = fs.mkdtempSync(path.join(os.tmpdir(), 'arc-grp-impact-int-'));
...
describe('group impact integration', () => {
  ...
  it('runs happy-path stub against fixture group (stops before bridge when no symbol UIDs)', async () => {
    const prev = process.env.ARC_HOME;
    process.env.ARC_HOME = tmpHome;
    try {
      const svc = new GroupService(stubPort());
      ...
      if (prev === undefined) delete process.env.ARC_HOME;
      else process.env.ARC_HOME = prev;
    }
  });
});
