...
describe('IncludeExtractor → syncGroup integration (finding #7)', () => {
  ...
  it('round-trip: extractor output from two real temp repos produces matching contract-ids', async () => {
    ...
    const providerDir = fs.mkdtempSync(path.join(os.tmpdir(), 'arc-include-int-provider-'));
    const consumerDir = fs.mkdtempSync(path.join(os.tmpdir(), 'arc-include-int-consumer-'));
    try {
