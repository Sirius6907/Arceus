/**
 * Smoke-test `arc group` CLI via tsx (same pattern as cli-e2e.test.ts).
...
beforeAll(() => {
  tmpHome = fs.mkdtempSync(path.join(os.tmpdir(), 'arc-group-cli-'));
});
...
function runGroup(args: string[]) {
  return spawnSync(process.execPath, ['--import', tsxImportUrl, cliEntry, 'group', ...args], {
    ...
    env: { ...process.env, ARC_HOME: tmpHome },
  });
}
...
describe('group CLI', () => {
  ...
  it('group impact runs with Issue #794 style flags (fixture-backed home)', () => {
    const home = fs.mkdtempSync(path.join(os.tmpdir(), 'arc-cli-impact-'));
    try {
