 ...
 * - analyzeCommand: verify pipeline runs and creates .arc/ output
 *
...
beforeAll(() => {
  ...
  suiteArceusHome = fs.mkdtempSync(path.join(os.tmpdir(), 'arc-cli-e2e-home-'));
...
afterAll(() => {
  ...
  if (suiteArceusHome) {
    fs.rmSync(suiteArceusHome, { recursive: true, force: true });
  }
});
 ...
 * isolate the global registry via ARC_HOME so they don't touch the
 * developer / CI agent's real ~/.arc/registry.json (#829).
 */
...
describe('CLI end-to-end', () => {
  ...
  it('analyze command runs pipeline on mini-repo', () => {
    ...
    // Successful analyze should create .arc/ output directory
    const arcDir = path.join(MINI_REPO, '.arc');
    expect(fs.existsSync(arcDir)).toBe(true);
    expect(fs.statSync(arcDir).isDirectory()).toBe(true);
    expect(fs.existsSync(path.join(MINI_REPO, '.gitignore'))).toBe(false);
    expect(fs.readFileSync(path.join(arcDir, '.gitignore'), 'utf-8')).toBe('*\n');
  }, 60_000);
  ...
  it('analyze persists meta.json AND a matching registry entry (#1169)', () => {
    ...
    try {
      const result = runCliWithEnv(['analyze'], repo, { ARC_HOME: gnHome }, 60000);
      ...
      const metaPath = path.join(repo, '.arc', 'meta.json');
      expect(
  ...
  it('already-up-to-date analyze fails when registry entry is missing (#1169)', () => {
    ...
    try {
      const first = runCliWithEnv(['analyze'], repo, { ARC_HOME: gnHome }, 60000);
      expect(
        first.status,
        [
      ...
      const second = runCliWithEnv(['analyze'], repo, { ARC_HOME: gnHome }, 60000);
      expect(
        second.status,
        [
  ...
  //   1. `analyze --name X` persists the alias to ~/.arc/registry.json
  ...
  describe('remove <target> (#664)', () => {
    it('dry-run lists, --force deletes, missing target is a no-op warning', () => {
      ...
      try {
        ...
        const r2 = runCliWithEnv(['remove', 'alias-a'], parentA, { ARC_HOME: gnHome }, 15000);
        if (r2.status === null) return;
        ...
        const r4 = runCliWithEnv(['remove', 'alias-a'], parentA, { ARC_HOME: gnHome }, 15000);
        if (r4.status === null) return;
    ...
    it('refuses to proceed when a registry entry points storagePath outside <repo>/.arc (#1003)', () => {
      // Regression guard for the safety gap flagged by @magyargergo on
      // PR #1003: `~/.arc/registry.json` is a user-writable JSON
      ...
      // `<entry.path>/.arc`. We verify:
      //   1. Exit code 1 with the actionable "registry entry corrupted"
      //      hint.
      //   2. The .arc/ storage dir is UNTOUCHED.
      ...
      try {
        ...
        expect(fs.existsSync(path.join(repo, '.arc'))).toBe(true);
        ...
        // Repo + .arc dir + .git dir must all still exist — the
        ...
        expect(fs.existsSync(path.join(repo, '.arc')), 'storage dir must survive').toBe(true);
  ...
  // the rest. We verify:
  //   1. The poisoned entry is NOT deleted (working tree + .arc
  ...
  //      .arc dir AND its registry entry are gone).
  describe('clean --all with a poisoned registry entry (#1003)', () => {
    it('skips poisoned entries, cleans valid ones, never deletes the working tree', () => {
      ...
      try {
        ...
        // Sanity: both working trees and .arc dirs still exist.
        expect(fs.existsSync(repoBad)).toBe(true);
        expect(fs.existsSync(path.join(repoBad, '.arc'))).toBe(true);
        expect(fs.existsSync(path.join(repoBad, '.git'))).toBe(true);
        expect(fs.existsSync(path.join(repoGood, '.arc'))).toBe(true);
        ...
        // Poisoned repo: working tree + .arc + .git all SURVIVE.
        expect(fs.existsSync(repoBad), 'poisoned repo working tree must survive').toBe(true);
        expect(
          fs.existsSync(path.join(repoBad, '.arc')),
          'poisoned repo .arc must survive (guard refused to rm repo root)',
        ).toBe(true);
  ...
  describe('unhappy path', () => {
    ...
    it('shows help with --help flag', () => {
      ...
      expect(result.stdout).toMatch(/arc/i);
  ...
  describe('CLI error handling', () => {
    ...
    it('status on non-indexed repo reports not indexed', () => {
      ...
      // in this suite may have created MINI_REPO/.arc via analyze,
      // and findRepo() walks up so any `.arc` along the path still
  ...
  describe('wiki command flags', () => {
    ...
    it('wiki on non-indexed repo fails with "No Arceus index"', () => {
      ...
      try {
        ...
        // Must spawn outside project tree so it doesn't find parent .arc
        ...
        expect(result.status).toBe(1);
        expect(result.stdout).toMatch(/No Arceus index found/);
      } finally {
        fs.rmSync(tmpDir, { recursive: true, force: true });
      }
    });
  ...
  // Requires analyze to have run first (the analyze test above populates .arc/).
    ...
    it('READY signal appears on stdout, not stderr', () => {
      return new Promise<void>((resolve, reject) => {
        ...
        child.stdout.on('data', (chunk: Buffer) => {
          ...
          if (stdoutBuffer.includes('ARC_EVAL_SERVER_READY:')) {
            foundOnStdout = true;
            child.kill('SIGTERM');
          }
        });
        ...
        child.stderr.on('data', (chunk: Buffer) => {
          ...
          if (text.includes('ARC_EVAL_SERVER_READY:')) {
            foundOnStderr = true;
            child.kill('SIGTERM');
          }
        });
