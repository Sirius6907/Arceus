...
describe('openLbugConnection — stale-sidecar sweep (test fixtures only)', () => {
  ...
  beforeEach(async () => {
    fixtureDir = await fs.mkdtemp(path.join(os.tmpdir(), 'arc-lbug-sweep-'));
  ...
  it('does not sweep production paths even if they share the prefix', async () => {
    ...
    await expect(openLbugConnection(stub, '/var/data/arc-lbug-fake/lbug')).rejects.toThrow(
...
describe('isTestFixturePath — production-safety guard', () => {
  it('accepts a fixture under os.tmpdir with a recognized prefix on the immediate parent', () => {
    const tmp = os.tmpdir();
    expect(isTestFixturePath(path.join(tmp, 'arc-lbug-XXX', 'lbug'))).toBe(true);
    expect(isTestFixturePath(path.join(tmp, 'arc-test-YYY', 'lbug'))).toBe(true);
  });

  it('rejects production paths even with a matching prefix', () => {
    expect(isTestFixturePath('/var/data/arc-lbug-fake/lbug')).toBe(false);
    expect(isTestFixturePath('/home/user/arc-test-foo/lbug')).toBe(false);
  });

  it('rejects path traversal attempts that resolve outside tmpdir', () => {
    const tmp = os.tmpdir();
    const traversal = path.join(tmp, 'arc-lbug-x', '..', '..', 'etc', 'passwd');
  ...
  it('rejects when the immediate parent does not match even if a deeper ancestor does', () => {
    ...
    // `<tmp>/arc-lbug-x/inner/lbug` to satisfy the predicate. We
    // require the immediate parent to match.
    const tmp = os.tmpdir();
    expect(isTestFixturePath(path.join(tmp, 'arc-lbug-x', 'inner', 'lbug'))).toBe(false);
  });
  ...
  it('handles tmpdir trailing-separator gracefully', () => {
    ...
    const tmp = os.tmpdir();
    const fixture = path.join(tmp, 'arc-lbug-trailing', 'lbug');
...
describe('waitForWindowsHandleRelease', () => {
  ...
  beforeEach(async () => {
    fixtureDir = await fs.mkdtemp(path.join(os.tmpdir(), 'arc-lbug-probe-'));
    dbPath = path.join(fixtureDir, 'lbug');
  });
