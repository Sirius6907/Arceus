 ...
 *
 * Tests the hook scripts with real git repos and .arc directories.
...
describe.each(HOOKS)('hooks e2e ($name)', ({ name, path: hookPath }) => {
  describe('PostToolUse staleness detection', () => {
    it('detects stale index when meta.json lastCommit differs from HEAD', () => {
      ...
      expect(output!.additionalContext).toContain('npx arc analyze');
    });
  ...
  describe('PreToolUse — silent without arc CLI', () => {
    // PreToolUse tries to spawn `arc augment` which won't be available in CI.
    ...
    it('handles Grep pattern gracefully when CLI is unavailable', () => {
      ...
      // spawned `arc augment` hangs and the 10s timeout kills the process.
  ...
  describe('unhappy paths', () => {
    it('handles corrupted meta.json (invalid JSON) without crashing', () => {
      fs.writeFileSync(path.join(arceusDir, 'meta.json'), 'THIS IS NOT JSON {{{');
    ...
    it('handles meta.json with missing lastCommit field', () => {
      fs.writeFileSync(path.join(arceusDir, 'meta.json'), JSON.stringify({ stats: {} }));
  ...
  describe('directory without .arc', () => {
    // The hook walks up 5 parent directories looking for .arc.
    // To guarantee none is found, create a deeply nested temp dir at the
    // filesystem root where no .arc could exist in any ancestor.
    let noArceusDir: string;

    beforeAll(() => {
      // Use a root-level temp path so parent traversal can't find .arc
      const root = os.platform() === 'win32' ? 'C:\\' : '/tmp';
      const base = path.join(root, `no-arc-${Date.now()}`);
      // Nest 6 levels deep (hook walks up 5) to ensure isolation
      noArceusDir = path.join(base, 'a', 'b', 'c', 'd', 'e', 'f');
      fs.mkdirSync(noArceusDir, { recursive: true });
      spawnSync('git', ['init'], { cwd: noArceusDir, stdio: 'pipe' });
    });
    ...
    afterAll(() => {
      ...
      const base = path.join(
        root,
        path.basename(path.resolve(noArceusDir, '..', '..', '..', '..', '..', '..')),
      );
      fs.rmSync(base, { recursive: true, force: true });
    });

    it('ignores PostToolUse when no .arc directory exists', () => {
      const result = runHook(hookPath, {
        hook_event_name: 'PostToolUse',
        tool_name: 'Bash',
        tool_input: { command: 'git commit -m "x"' },
    ...
    it('ignores PreToolUse when no .arc directory exists', () => {
      const result = runHook(hookPath, {
        hook_event_name: 'PreToolUse',
        tool_name: 'Grep',
