...
describe('filesystem-walker', () => {
  ...
  describe('.arcignore support', () => {
    ...
    beforeAll(async () => {
      ...
      // Only .arcignore, no .gitignore
      await fs.writeFile(path.join(nexusignoreDir, '.arcignore'), 'local/\n');
    });
    ...
    it('excludes directories listed in .arcignore', async () => {
  ...
  describe('combined .gitignore + .arcignore', () => {
    ...
    beforeAll(async () => {
      ...
      await fs.writeFile(path.join(combinedDir, '.arcignore'), 'local/\n');
    });
  ...
  describe('ARC_NO_GITIGNORE env var', () => {
    let envDir: string;
    ...
    it('includes gitignored directory when ARC_NO_GITIGNORE is set', async () => {
  ...
  describe('large file skip threshold (#991)', () => {
    ...
    const ORIGINAL_ENV = process.env.ARC_MAX_FILE_SIZE;
    ...
    beforeEach(() => {
      delete process.env.ARC_MAX_FILE_SIZE;
    ...
    afterEach(() => {
      if (ORIGINAL_ENV === undefined) {
        ...
        process.env.ARC_MAX_FILE_SIZE = ORIGINAL_ENV;
      }
      cap.restore();
    });
    ...
    it('includes the 600KB file when ARC_MAX_FILE_SIZE=1024', async () => {
      process.env.ARC_MAX_FILE_SIZE = '1024';
      const files = await walkRepositoryPaths(sizeDir);
      const paths = files.map((f) => f.path.replace(/\\/g, '/'));
      expect(paths).toContain(BIG_FILE);
    });

    it('falls back to default and warns once on invalid ARC_MAX_FILE_SIZE', async () => {
      process.env.ARC_MAX_FILE_SIZE = 'abc';
      const files = await walkRepositoryPaths(sizeDir);
      const paths = files.map((f) => f.path.replace(/\\/g, '/'));
      expect(paths).not.toContain(BIG_FILE);
