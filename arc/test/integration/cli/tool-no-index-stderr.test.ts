 ...
 *
 * Symptom before the fix: `arc tool query <foo>` with no indexed
 ...
 * `arc/src/cli/cli-message.ts`), which writes plain text directly
...
/**
 * Spawn the built `arc` CLI with arguments, wait for exit, and
 * return captured streams + exit code. Pin ARC_HOME to a fresh
...
function runCli(args: string[]): Promise<ChildResult> {
  const tmpHome = fs.mkdtempSync(path.join(os.tmpdir(), 'arc-cli-no-index-'));
...
describe('CLI tool query — diagnostic survives hard exit (plan 002)', () => {
  it('emits the no-index diagnostic to stderr before exit code 1', async () => {
    ...
    expect(result.stderr).toContain('arc analyze');
    ...
    // for stdout (e.g., `arc query | jq`); diagnostics are stderr.
