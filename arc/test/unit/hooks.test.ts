...
const CJS_HOOK = path.resolve(__dirname, '..', '..', 'hooks', 'claude', 'arc-hook.cjs');
...
// ─── Test fixtures: temporary .arc directory ───────────────────

let tmpDir: string;
let arceusDir: string;

beforeAll(() => {
  tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'arc-hook-test-'));
  arceusDir = path.join(tmpDir, '.arc');
  fs.mkdirSync(arceusDir, { recursive: true });
...
function createGlobalRegistry(homeDir: string, marker: 'both' | 'registry' | 'repos' = 'both') {
  const registryDir = path.join(homeDir, '.arc');
...
describe('Windows .cmd extension handling', () => {
  ...
  it('Plugin hook uses .cmd extension for Windows arc binary', () => {
    const source = fs.readFileSync(PLUGIN_HOOK, 'utf-8');
    expect(source).toContain('arc.cmd');
  });
});
...
describe('PreToolUse concurrency guard', () => {
  for (const [label, hookPath, lockPath] of [
    ...
    it(`${label} hook fails closed when lock dir cannot be created`, () => {
      ...
      // cross-user `.arc/` setups. The guard must fail closed (null).
      const source = fs.readFileSync(lockPath, 'utf-8');
      const slotFn = source.slice(
        source.indexOf('function acquireHookSlot'),
        source.indexOf('function', source.indexOf('function acquireHookSlot') + 1),
...
describe('PreToolUse concurrency guard (integration)', () => {
  for (const [label, hookPath] of [
    ...
    it(`${label}: hook exits silently when all MAX_INFLIGHT slots hold live pids`, async () => {
      ...
      const lockDir = path.join(arceusDir, '.hook-locks');
    ...
    it(`${label}: hook does not exceed MAX_INFLIGHT under simultaneous bursts (hard cap)`, async () => {
      ...
      // without spawning the real arc augment CLI.
      const claimerScript = `
...
describe('Global registry lookup', () => {
  for (const [label, hookPath] of [
    ...
    it(`${label}: PostToolUse stays silent for unindexed repo under global registry`, () => {
      const homeDir = fs.mkdtempSync(path.join(os.tmpdir(), 'arc-home-'));
    ...
    it(`${label}: PostToolUse emits stale for indexed repo under parent global registry`, () => {
      ...
      try {
        createGlobalRegistry(homeDir);
        fs.mkdirSync(path.join(repoDir, '.arc'), { recursive: true });
        initGitRepo(repoDir);
        fs.writeFileSync(
          path.join(repoDir, '.arc', 'meta.json'),
          JSON.stringify({ lastCommit: 'oldcommit', stats: {} }),
        );
...
describe('Linked git worktree resolution', () => {
  for (const [label, hookPath] of [
    ...
    it(`${label}: PostToolUse emits stale from a linked worktree pointing at an indexed canonical repo`, () => {
      ...
      //   <root>/main-repo/.arc/        (only here)
      //   <root>/main-repo-worktrees/feat/   (linked worktree, no .arc)
      const root = fs.mkdtempSync(path.join(os.tmpdir(), 'arc-worktree-'));
      ...
      try {
        ...
        fs.mkdirSync(path.join(mainRepo, '.arc'), { recursive: true });
        fs.writeFileSync(
          path.join(mainRepo, '.arc', 'meta.json'),
          JSON.stringify({ lastCommit: 'oldcommit', stats: {} }),
        );
        ...
        // Sanity: walking up from the worktree never reaches `.arc`.
        expect(fs.existsSync(path.join(worktreePath, '.arc'))).toBe(false);
        expect(fs.existsSync(path.join(path.dirname(worktreePath), '.arc'))).toBe(false);
    ...
    it(`${label}: PostToolUse silent from a linked worktree when canonical repo has no .arc`, () => {
      const root = fs.mkdtempSync(path.join(os.tmpdir(), 'arc-worktree-'));
      const mainRepo = path.join(root, 'main-repo');
      const worktreePath = path.join(root, 'main-repo-worktrees', 'feat');
      try {
        fs.mkdirSync(mainRepo, { recursive: true });
        initGitRepo(mainRepo);
        // Note: NO .arc/ in the canonical repo.
...
describe('PostToolUse with missing/corrupt meta.json', () => {
  for (const [label, hookPath] of [
    ...
    it(`${label}: emits stale when meta.json does not exist`, () => {
      const metaPath = path.join(arceusDir, 'meta.json');
