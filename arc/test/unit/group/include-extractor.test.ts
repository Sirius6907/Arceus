...
describe('IncludeExtractor', () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'arc-include-'));
...
int main() { return 0; }`,
  ...
  describe('cross-repo matching', () => {
    it('provider and consumer produce matching contractIds', async () => {
      ...
      const providerDir = fs.mkdtempSync(path.join(os.tmpdir(), 'arc-include-provider-'));
      ...
      const consumerDir = fs.mkdtempSync(path.join(os.tmpdir(), 'arc-include-consumer-'));
...
int main(){return 0;}`,
  ...
  describe('follow-up: extractProvidersGraph strips repo root from absolute paths', () => {
    it('produces repo-relative contract IDs when the graph returns absolute paths', async () => {
      ...
      // arc analyze actually persists them.
  ...
  describe('follow-up: file discovery honors createIgnoreFilter and getMaxFileSizeBytes', () => {
    ...
    it('does not emit a provider contract for a header excluded by .arcignore', async () => {
      writeFile('.arcignore', 'legacy/\n');
...
int auto_main() { return 0; }`,
    ...
    it('skips a provider header whose size exceeds ARC_MAX_FILE_SIZE', async () => {
      const previous = process.env.ARC_MAX_FILE_SIZE;
      process.env.ARC_MAX_FILE_SIZE = '1'; // 1 KB cap
      try {
        ...
        if (previous === undefined) delete process.env.ARC_MAX_FILE_SIZE;
        else process.env.ARC_MAX_FILE_SIZE = previous;
      }
    });

    it('skips parsing #include directives in source files exceeding ARC_MAX_FILE_SIZE', async () => {
      const previous = process.env.ARC_MAX_FILE_SIZE;
      process.env.ARC_MAX_FILE_SIZE = '1';
      try {
        const oversized =
          '#include "remote/should_not_appear.h"\n' +
  ...
  describe('Windows SIGSEGV regression — large input must route through parseSourceSafe', () => {
    it('routes >32 767-char header file through parseSourceSafe (not direct parser.parse)', async () => {
      ...
      const previousLimit = process.env.ARC_MAX_FILE_SIZE;
      process.env.ARC_MAX_FILE_SIZE = '512';
      try {
        ...
        if (previousLimit === undefined) delete process.env.ARC_MAX_FILE_SIZE;
        else process.env.ARC_MAX_FILE_SIZE = previousLimit;
      }
    });
  });
});
