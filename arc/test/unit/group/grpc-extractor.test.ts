...
describe('GrpcExtractor', () => {
  ...
  beforeEach(async () => {
    tmpDir = await fsp.mkdtemp(path.join(os.tmpdir(), 'arc-grpc-'));
...
export class AuthGateway {
  ...
  // ─── #1185: gRPC extractor must honour .arcignore ──────────────
  //
  ...
  // `filesystem-walker.ts`) so any `.arcignore` pattern is
  ...
  describe('respects .arcignore (#1185)', () => {
    it('proto + source globs both skip files matched by .arcignore', async () => {
...
service LeakedService {
  rpc Ping (PingRequest) returns (PingResponse);
      ...
      // `.arcignore` filter this WOULD emit a `grpc::*/LeakedService`
      // consumer contract.
      writeFile(
...
stub = leaked_pb2_grpc.LeakedServiceStub(channel)`,
      ...
      writeFile('.arcignore', 'mentor_env/\n');
...
describe('GrpcExtractor.extract ambiguous proto resolution', () => {
  ...
  beforeEach(async () => {
    tmpDir = await fsp.mkdtemp(path.join(os.tmpdir(), 'arc-grpc-ambig-'));
    extractor = new GrpcExtractor();
  });
  afterEach(async () => {
    await fsp.rm(tmpDir, { recursive: true, force: true });
