...
describe('TopicExtractor', () => {
  ...
  beforeEach(() => {
    tmpDir = path.join(os.tmpdir(), `arc-topic-${Date.now()}`);
...
await consumer.subscribe({ topic: 'order.placed' });`,
  ...
  // ─── #1185: topic extractor must honour .arcignore ─────────────
  //
  ...
  // so any `.arcignore` pattern excludes those files from contract
...
public void handleLeaked(ConsumerRecord<String, String> r) {}`,
    ...
    it('still prunes `*_test.go` even when .arcignore is empty (wrapper preserves glob-level filter)', async () => {
