...
describe('HttpRouteExtractor', () => {
  ...
  beforeEach(() => {
    ...
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'arc-http-extract-'));
  });
...
`,
  ...
  // `.arcignore` (or `.gitignore`) prunes the glob.
  describe('respects .arcignore (#1185)', () => {
    it('source-scan glob skips files matched by .arcignore', async () => {
      const dir = path.join(tmpDir, 'arcignore-honoured');
      fs.mkdirSync(path.join(dir, 'src/routes'), { recursive: true });
...
`,
      ...
      fs.writeFileSync(path.join(dir, '.arcignore'), 'mentor_env/\n');
    ...
    // Pinned by the @claude review on PR #1247: above, only `.arcignore`
    ...
    it('source-scan glob also skips files matched by `.gitignore` (no `.arcignore`)', async () => {
...
`,
      ...
      // Note: NO .arcignore — only `.gitignore`. This proves the
