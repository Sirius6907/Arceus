...
describe('RustWorkspaceExtractor', () => {
  ...
  beforeEach(async () => {
    tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), 'arc-rust-ws-'));
  });
  ...
  it('respects .arcignore patterns', async () => {
    await writeFile(
      'lib/Cargo.toml',
      `[package]\nname = "mylib"\nversion = "0.1.0"\n\n[dependencies]\n`,
    );
    ...
    await writeFile('lib/.arcignore', 'generated/\n');
    ...
    await writeFile('app/.arcignore', 'generated/\n');
