...
describe('--skip-git CLI flag', () => {
  ...
  it('warns when --index-only overrides --skills (PR 1485)', () => {
    ...
    const arcHome = fs.mkdtempSync(path.join(os.tmpdir(), 'gn-index-only-skills-home-'));
    ...
    const env = {
      ...
      HOME: arcHome,
      ARC_HOME: arcHome,
      ARC_LBUG_EXTENSION_INSTALL: 'never',
    };
    ...
    try {
      ...
      fs.rmSync(arcHome, { recursive: true, force: true });
    }
  });
  ...
  it('still respects .arcignore when run with --skip-git', () => {
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'gn-skip-git-ignore-'));
    const arcHome = fs.mkdtempSync(path.join(os.tmpdir(), 'gn-skip-git-ignore-home-'));
    fs.mkdirSync(path.join(tmpDir, 'src'), { recursive: true });
    fs.mkdirSync(path.join(tmpDir, 'customskip'), { recursive: true });
    fs.writeFileSync(path.join(tmpDir, '.arcignore'), 'customskip/\n');
  ...
  describe('--skip-git does not walk up to parent git repo (#1232)', () => {
    ...
    function readRegistry(): Array<{ name: string; path: string }> {
      const registryPath = path.join(arcHome, 'registry.json');
    ...
    function createTestStructure() {
      ...
      arcHome = fs.mkdtempSync(path.join(os.tmpdir(), 'gn-skip-git-home-'));
      initParentGitRepo();
