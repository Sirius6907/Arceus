...
describe('getStoragePath', () => {
  it('appends .arc to resolved repo path', () => {
    const result = getStoragePath('/home/user/project');
    expect(result).toContain('.arc');
    expect(path.basename(result)).toBe('.arc');
  });
...
describe('getStoragePaths', () => {
  it('returns storagePath, lbugPath, metaPath', () => {
    ...
    expect(paths.storagePath).toContain('.arc');
...
// ─── Arceus ignore rules (#1233) ─────────────────────────────────────

describe('ensureArceusIgnored (#1233)', () => {
  let tmpRepo: Awaited<ReturnType<typeof createTempDir>>;

  beforeEach(async () => {
    tmpRepo = await createTempDir('arc-internal-gitignore-');
  });
  ...
  it('creates .arc/.gitignore containing a catch-all ignore rule', async () => {
    await ensureArceusIgnored(tmpRepo.dbPath);

    await expect(
      fs.readFile(path.join(tmpRepo.dbPath, '.arc', '.gitignore'), 'utf-8'),
    ).resolves.toBe('*\n');
  });
  ...
  it('adds .arc/ to .git/info/exclude when the repo has a real .git directory', async () => {
    const excludePath = path.join(tmpRepo.dbPath, '.git', 'info', 'exclude');
    await fs.mkdir(path.dirname(excludePath), { recursive: true });

    await ensureArceusIgnored(tmpRepo.dbPath);

    await expect(fs.readFile(excludePath, 'utf-8')).resolves.toBe('.arc/\n');
  });

  it('appends .arc/ to .git/info/exclude once without disturbing existing rules', async () => {
    const excludePath = path.join(tmpRepo.dbPath, '.git', 'info', 'exclude');
    await fs.mkdir(path.dirname(excludePath), { recursive: true });
  ...
  it('keeps generated .arc files out of git status', async () => {
    ...
    await ensureArceusIgnored(tmpRepo.dbPath);
    await fs.writeFile(path.join(tmpRepo.dbPath, '.arc', 'meta.json'), '{}\n');
...
describe('readRegistry', () => {
  it('returns empty array when registry does not exist', async () => {
    // readRegistry reads from ~/.arc/registry.json
...
describe('saveCLIConfig / loadCLIConfig', () => {
  ...
  beforeEach(async () => {
    tmpHandle = await createTempDir('arc-config-test-');
...
describe('API key file permissions', () => {
  it('saveCLIConfig calls chmod 0o600 on non-Windows', async () => {
    ...
    const source = await fs.readFile(
      new URL('../../src/storage/repo-manager.ts', import.meta.url),
      'utf-8',
    );
...
//
// Each test isolates the global registry by pointing ARC_HOME at a
...
describe('registerRepo name override + collision guard (#829)', () => {
  ...
  let savedArceusHome: string | undefined;
  ...
  beforeEach(async () => {
    tmpHome = await createTempDir('arc-registry-home-');
    tmpRepoA = await createTempDir('arc-repo-a-');
    tmpRepoB = await createTempDir('arc-repo-b-');
    savedArceusHome = process.env.ARC_HOME;
    process.env.ARC_HOME = tmpHome.dbPath;
  });

  afterEach(async () => {
    if (savedArceusHome === undefined) delete process.env.ARC_HOME;
    else process.env.ARC_HOME = savedArceusHome;
  ...
  it('basename collisions without an explicit --name still register silently (backward-compat)', async () => {
    ...
    const parentA = await createTempDir('arc-collide-parent-a-');
    const parentB = await createTempDir('arc-collide-parent-b-');
...
describe('getInferredRepoName + registerRepo (#979 — git remote inference)', () => {
  ...
  beforeEach(async () => {
    tmpHome = await createTempDir('arc-registry-home-979-');
    savedArceusHome = process.env.ARC_HOME;
    process.env.ARC_HOME = tmpHome.dbPath;
  });
  ...
  it('getInferredRepoName returns null when there is no .git directory', async () => {
    const tmp = await createTempDir('arc-no-git-');
    try {
  ...
  it('getInferredRepoName returns null when origin is unset', async () => {
    const tmp = await createTempDir('arc-no-origin-');
    try {
  ...
  it('getInferredRepoName returns the remote repo name when origin is set', async () => {
    const tmp = await createTempDir('arc-with-origin-');
    try {
  ...
  it('registerRepo derives name from git remote when basename is generic (Gas-Town repro)', async () => {
    ...
    const root = await createTempDir('arc-gastown-');
    try {
  ...
  it('two analyze calls of differently-remoted "rig" leaves no longer collide', async () => {
    ...
    const root = await createTempDir('arc-gastown-2-');
    try {
  ...
  it('explicit --name still wins over remote inference', async () => {
    const tmp = await createTempDir('arc-name-wins-');
    try {
  ...
  it('preserved alias still wins over remote inference on re-analyze', async () => {
    const tmp = await createTempDir('arc-preserve-alias-');
    try {
      initGitRepo(tmp.dbPath, 'https://github.com/owner/from-remote.git');
  ...
  it('falls back to basename when no .git / no remote is available', async () => {
    const tmp = await createTempDir('arc-fallback-basename-');
    try {
      const name = await registerRepo(tmp.dbPath, meta);
...
// ─── resolveRegistryEntry (#664 — arc remove <target>) ──────────
//
...
// ~/.arc. No ARC_HOME sandboxing needed. This also means the
  ...
  const entries: RegistryEntry[] = [
    {
      name: 'app',
      path: pathA,
      storagePath: `${pathA}${path.sep}.arc`,
    ...
    },
    {
      name: 'app',
      path: pathB,
      storagePath: `${pathB}${path.sep}.arc`,
      indexedAt: '2026-04-18T00:00:00.000Z',
      lastCommit: 'bbbbbbb',
    },
    {
      name: 'website',
      path: pathW,
      storagePath: `${pathW}${path.sep}.arc`,
      indexedAt: '2026-04-18T00:00:00.000Z',
      lastCommit: 'ccccccc',
    },
  ];
  ...
  it('path match wins over name match (never ambiguous)', () => {
    ...
    const weird: RegistryEntry[] = [
      { ...entries[2] }, // 'website' at pathW
      {
        ...
        storagePath: `${prefix}elsewhere${path.sep}odd${path.sep}.arc`,
        indexedAt: '2026-04-18T00:00:00.000Z',
        lastCommit: 'ddddddd',
      },
    ];
...
describe('resolveRegistryEntry backward-compat with non-canonical stored paths (#1003)', () => {
  it('matches a stored entry even when the target was passed in canonical form', async () => {
    ...
    const nonCanonical = realDir + path.sep + '.'; // e.g. /work/arc/.
    ...
    const entries: RegistryEntry[] = [
      {
        ...
        path: nonCanonical,
        storagePath: path.join(nonCanonical, '.arc'),
...
//
// Guard rail against destroying more than the `.arc/` subfolder.
// `~/.arc/registry.json` is user-writable plain text, so a
...
describe('assertSafeStoragePath (#1003)', () => {
  ...
  it('accepts the canonical <repo>/.arc storage path', () => {
    const entry: RegistryEntry = {
      ...base,
      storagePath: path.join(repoPath, '.arc'),
    };
  ...
  it('rejects when storagePath is a sibling .arc (right basename, wrong parent)', () => {
    const entry: RegistryEntry = {
      ...base,
      storagePath: path.join(`${prefix}different${path.sep}repo`, '.arc'),
    };
    expect(() => assertSafeStoragePath(entry)).toThrow(UnsafeStoragePathError);
  });
  ...
  it('UnsafeStoragePathError carries the original entry + expected + actual paths', () => {
    ...
    try {
      ...
      // Expected path is the canonical `<repo>/.arc`.
      expect(err.expectedStoragePath).toBe(path.join(path.resolve(repoPath), '.arc'));
  ...
  it('Windows: storagePath match is case-insensitive to match register/unregister semantics', () => {
    ...
    const entry: RegistryEntry = {
      ...base,
      storagePath: path.join(repoPath.toUpperCase(), '.ARC'),
    };
...
describe('registerRepo worktree-aware basename fallback (#1259)', () => {
  ...
  beforeEach(async () => {
    tmpHome = await createTempDir('arc-registry-home-');
    tmpRepo = await createTempDir('arc-canonical-repo-');
    savedArceusHome = process.env.ARC_HOME;
    process.env.ARC_HOME = tmpHome.dbPath;
  });
