...
const ORIGINAL_TIMEOUT = process.env.ARC_WORKER_SUB_BATCH_TIMEOUT_MS;
const ORIGINAL_MAX_BYTES = process.env.ARC_WORKER_SUB_BATCH_MAX_BYTES;

describe('resolveWorkerPoolOptions', () => {
  afterEach(() => {
    if (ORIGINAL_TIMEOUT === undefined) {
      delete process.env.ARC_WORKER_SUB_BATCH_TIMEOUT_MS;
    } else {
      process.env.ARC_WORKER_SUB_BATCH_TIMEOUT_MS = ORIGINAL_TIMEOUT;
    }

    if (ORIGINAL_MAX_BYTES === undefined) {
      delete process.env.ARC_WORKER_SUB_BATCH_MAX_BYTES;
    } else {
      process.env.ARC_WORKER_SUB_BATCH_MAX_BYTES = ORIGINAL_MAX_BYTES;
    }
  });

  it('reads worker timeout and byte budget from environment variables', () => {
    process.env.ARC_WORKER_SUB_BATCH_TIMEOUT_MS = '5000';
    process.env.ARC_WORKER_SUB_BATCH_MAX_BYTES = '4096';
