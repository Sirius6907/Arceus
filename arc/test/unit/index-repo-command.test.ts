...
const mockEnsureArceusIgnored = vi.fn();
...
vi.mock('../../src/storage/repo-manager.js', () => ({
  ...
  ensureArceusIgnored: mockEnsureArceusIgnored,
}));
...
vi.mock('../../src/storage/git.js', () => ({
  ...
  // older `.arc/meta.json` files. The unit tests don't care
...
describe('indexCommand', () => {
  ...
  beforeEach(() => {
    ...
    mockGetStoragePaths.mockImplementation((repoPath: string) => ({
      storagePath: `${repoPath}/.arc`,
      lbugPath: `${repoPath}/.arc/lbug`,
      metaPath: `${repoPath}/.arc/meta.json`,
    }));
    ...
    mockEnsureArceusIgnored.mockResolvedValue(undefined);
  ...
  it('fails when .arc folder does not exist', async () => {
    const logSpy = vi.spyOn(console, 'log').mockImplementation(() => {});
    mockAccess.mockRejectedValueOnce(new Error('missing .arc'));
  ...
  it('registers successfully with existing metadata', async () => {
    ...
    expect(mockEnsureArceusIgnored).toHaveBeenCalledTimes(1);
    expect(mockEnsureArceusIgnored).toHaveBeenCalledWith(resolvedRepo);
    expect(process.exitCode).toBeUndefined();
  });
  ...
  it('fails when multiple path parts do not resolve to a single existing path', async () => {
    ...
    expect(mockRegisterRepo).not.toHaveBeenCalled();
    expect(mockEnsureArceusIgnored).not.toHaveBeenCalled();
    expect(process.exitCode).toBe(1);
