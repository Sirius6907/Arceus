...
// ─── .arcignore negation can override hardcoded list (#771) ────
//
// Per @magyargergo's review: `.arcignore` should honour
// `.gitignore`-style negation against the hardcoded DEFAULT_IGNORE_LIST.
// A `!__tests__/` line in `.arcignore` must re-enable indexing of
...
//
// Locks in:
//   1. Default (no .arcignore) — hardcoded list still blocks
...
describe('.arcignore negation overrides hardcoded DEFAULT_IGNORE_LIST (#771)', () => {
  let tmpDir: string;
  ...
  it('default (no .arcignore): __tests__ still blocked by hardcoded list', async () => {
  ...
  it('`!__tests__/` negation unlocks the directory and its descendants', async () => {
    await fs.writeFile(path.join(tmpDir, '.arcignore'), '!__tests__/\n');
    const filter = await createIgnoreFilter(tmpDir);
  ...
  it('`!__mocks__/` negation unlocks __mocks__ but NOT __tests__', async () => {
    await fs.writeFile(path.join(tmpDir, '.arcignore'), '!__mocks__/\n');
    const filter = await createIgnoreFilter(tmpDir);
  ...
  it('negation generalises — `!node_modules/` unlocks a different hardcoded entry', async () => {
    ...
    await fs.writeFile(path.join(tmpDir, '.arcignore'), '!node_modules/\n');
    const filter = await createIgnoreFilter(tmpDir);
  ...
  it('standard `.gitignore` rules (no negation) still layer on top of hardcoded', async () => {
    // Pre-#771 behaviour: if .arcignore says `my-dir/`, that dir
    // is ignored in addition to the hardcoded list. Non-negation
    // rules are unaffected by this PR.
    await fs.writeFile(path.join(tmpDir, '.arcignore'), 'my-dir/\n');
    const filter = await createIgnoreFilter(tmpDir);
  ...
  it('shouldIgnorePath (raw hardcoded check) is unchanged — wiki / external callers unaffected', async () => {
    ...
    // doesn't have access to per-repo `.arcignore` config. Its
...
describe('loadIgnoreRules', () => {
  ...
  it('parses .arcignore file', async () => {
    await fs.writeFile(path.join(tmpDir, '.arcignore'), 'vendor/\n*.test.ts\n');
    const ig = await loadIgnoreRules(tmpDir);
    expect(ig).not.toBeNull();
    ...
    await fs.unlink(path.join(tmpDir, '.arcignore'));
  });
...
describe('createIgnoreFilter', () => {
  ...
  it('childrenIgnored respects negation patterns (exclude-all + whitelist)', async () => {
    // Reproduces https://github.com/Sirius6907/Arceus/issues/596
    // Pattern: `*` (exclude all) + `!iOS/` + `!iOS/**` (whitelist iOS)
    await fs.writeFile(
      path.join(tmpDir, '.arcignore'),
      '*\n!iOS/\n!iOS/**\n!backend/\n!backend/living_plan/\n!backend/living_plan/**\n',
    );
    const filter = await createIgnoreFilter(tmpDir);
  ...
  it('childrenIgnored respects negation patterns without trailing slash (!dir vs !dir/)', async () => {
    ...
    await fs.writeFile(path.join(tmpDir, '.arcignore'), '*\n!iOS\n!iOS/**\n');
    const filter = await createIgnoreFilter(tmpDir);
...
describe('loadIgnoreRules — ARC_NO_GITIGNORE env var', () => {
  let tmpDir: string;
  ...
  it('skips .gitignore when ARC_NO_GITIGNORE is set', async () => {
    await fs.writeFile(path.join(tmpDir, '.gitignore'), 'data/\n');

    const original = process.env.ARC_NO_GITIGNORE;
    process.env.ARC_NO_GITIGNORE = '1';
    try {
      const ig = await loadIgnoreRules(tmpDir);
      ...
      if (original === undefined) {
        delete process.env.ARC_NO_GITIGNORE;
      } else {
        process.env.ARC_NO_GITIGNORE = original;
      }
      await fs.unlink(path.join(tmpDir, '.gitignore'));
    }
  });

  it('still reads .arcignore when ARC_NO_GITIGNORE is set', async () => {
    await fs.writeFile(path.join(tmpDir, '.arcignore'), 'vendor/\n');
