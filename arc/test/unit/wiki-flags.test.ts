...
describe('resolveLLMConfig', () => {
  ...
  beforeEach(async () => {
    ...
    const configDir = path.join(tmpDir, '.arc');
...
describe('--verbose flag', () => {
  const originalEnv = process.env.ARC_VERBOSE;

  afterEach(() => {
    if (originalEnv === undefined) {
      delete process.env.ARC_VERBOSE;
    } else {
      process.env.ARC_VERBOSE = originalEnv;
    }
  });

  it('verboseLog writes to console when ARC_VERBOSE=1', async () => {
    ...
    expect(process.env.ARC_VERBOSE).toBe('1');

    consoleSpy.mockRestore();
  });

  it('verbose is off when ARC_VERBOSE is not set', () => {
    delete process.env.ARC_VERBOSE;
    expect(process.env.ARC_VERBOSE).toBeUndefined();
  });
});
