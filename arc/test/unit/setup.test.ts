...
// By default, execFileSync throws (simulating `which arc` not found)
...
describe('setupClaudeCode', () => {
  ...
  it('writes win32 MCP entry with cmd wrapper', async () => {
    ...
    expect(config.mcpServers.arc).toEqual({
      command: 'cmd',
  ...
  it('uses global binary path when arc is on PATH', async () => {
    setPlatform('darwin');
    ...
    expect(config.mcpServers.arc).toEqual({
      command: '/usr/local/bin/arc',
      args: ['mcp'],
    });
  });

  it('falls back to npx when arc is not on PATH', async () => {
    setPlatform('darwin');
  ...
  it('picks .cmd wrapper from Windows where output (multiple lines)', async () => {
    setPlatform('win32');
    // `where arc` on Windows returns the POSIX script first, then .cmd
