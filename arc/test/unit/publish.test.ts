...
describe('understand-quickly helpers (arceus-shared)', () => {
  describe('isValidOwnerRepo', () => {
    it.each([
      ['looptech-ai/understand-quickly', true],
      ['Sirius6907/Arceus', true],
  ...
  describe('parseOwnerRepoFromRemote', () => {
    it.each([
      ...
      ['ssh://git@github.com/Sirius6907/Arceus.git', 'Sirius6907/Arceus'],
...
describe('publishCommand (no-token no-op)', () => {
  ...
  beforeEach(async () => {
    ...
    await fs.mkdir(path.join(tempDir, '.arc'), { recursive: true });
    await fs.writeFile(
      path.join(tempDir, '.arc', 'meta.json'),
  ...
  it('exits 0 with no token even when no index/repo exists (BLOCKER 2)', async () => {
    ...
    // command must be a no-op even if the repo lacks `.arc/`.
