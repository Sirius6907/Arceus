 ...
 *
 * Tests the hook script at arceus-cursor-integration/hooks/arc-hook.cjs
 * which runs as a Cursor 2.4 postToolUse hook.
 *
 * Covers:
 * - extractPattern: pattern extraction from Grep/Read/Shell tool inputs
 * - findArceusDir: .arc directory discovery (shared with Claude hook)
...
// `.arc/` so the hook reaches acquireHookSlot. The base tmpDir above
// deliberately has no .arc so unrelated early-exit tests stay cheap.
let guardTmpDir: string;
let guardArceusDir: string;

beforeAll(() => {
  tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'arc-cursor-hook-test-'));
  ...
  guardTmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'arc-cursor-hook-guard-'));
  guardArceusDir = path.join(guardTmpDir, '.arc');
  fs.mkdirSync(guardArceusDir, { recursive: true });
...
describe('hooks.json wiring', () => {
  ...
  it('points command at the new Node hook', () => {
    ...
    expect(command).toContain('arc-hook.cjs');
...
describe('Cursor hook source regressions', () => {
  ...
  it('passes pattern after end-of-options marker (--)', () => {
    ...
    // otherwise be parsed as CLI flags by the arc CLI.
    expect(source).toMatch(/'augment',\s*'--',\s*pattern/);
  });

  it('gates on a non-global .arc directory before invoking the CLI', () => {
    expect(source).toContain('findArceusDir');
...
describe('Cursor hook extractPattern coverage', () => {
  ...
  it('logs raw payload to stderr when ARC_DEBUG is set (for contract diagnostics)', () => {
    expect(source).toContain('ARC_DEBUG');
    expect(source).toContain('Arceus Cursor hook stdin:');
  });
});
...
describe('Cursor hook behavior — early-exit paths', () => {
  ...
  it('produces no output when cwd has no .arc dir', () => {
    const result = runHook(CURSOR_HOOK, {
      tool_name: 'Grep',
      tool_input: { query: 'validateUser' },
      cwd: tmpDir,
...
// ─── Behavior: ARC_DEBUG payload logging ────────────────────────

describe('Cursor hook debug logging', () => {
  it('echoes the payload to stderr only when ARC_DEBUG is set', () => {
    const payload = {
      tool_name: 'Grep',
      tool_input: { query: 'validateUser' },
      cwd: tmpDir,
    ...
    expect(quiet.stderr).not.toContain('Arceus Cursor hook stdin');

    // ARC_DEBUG=1 → payload echoed to stderr (stdout still empty for
    ...
    expect(verbose.stderr).toContain('Arceus Cursor hook stdin');
...
describe('Cursor hook concurrency guard', () => {
  ...
  it('fails closed when lock dir cannot be created', () => {
    ...
    // a read-only or cross-user `.arc/` cannot reintroduce #1486.
    const slotFn = lockSource.slice(
      lockSource.indexOf('function acquireHookSlot'),
      lockSource.indexOf('function', lockSource.indexOf('function acquireHookSlot') + 1),
    );
...
describe('Cursor hook concurrency guard (integration)', () => {
  it('exits silently when all MAX_INFLIGHT slots hold live pids', async () => {
    ...
    const lockDir = path.join(guardArceusDir, '.hook-locks');
...
describe('Shell quoted-pattern parser limitations (documented)', () => {
  ...
  it('quoted multi-word `rg "User Service"` extracts the first word only', () => {
    const result = runHook(CURSOR_HOOK, {
      ...
      cwd: tmpDir, // no .arc → exits early after extract
    });
    expect(result.status).toBe(0);
    expect(result.stdout.trim()).toBe('');
  });
...
describe('Cursor integration install docs', () => {
  ...
  it('install README documents the hook install path', () => {
    ...
    expect(body).toContain('hooks/arc-hook.cjs');
    expect(body).toContain('hooks/hook-lock.cjs');
    expect(body).toContain('Hook install');
  });

  it('install README documents ARC_DEBUG for payload diagnostics', () => {
    const body = fs.readFileSync(integrationReadme, 'utf-8');
    expect(body).toContain('ARC_DEBUG');
  });
});
