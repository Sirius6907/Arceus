...
const MOCK_REPO_ENTRY = {
  ...
  storagePath: '/tmp/.arc/test-project',
...
function setupMultipleRepos() {
  (listRegisteredRepos as any).mockResolvedValue([
    MOCK_REPO_ENTRY,
    {
      ...
      storagePath: '/tmp/.arc/other-project',
    },
  ]);
}
...
describe('LocalBackend.callTool', () => {
  ...
  it('includes FTS-unavailable warning when ftsAvailable is false (#1403)', async () => {
    ...
    expect((result as any).warning).toMatch(/arc analyze --force/);
  });
        ...
        cap
          ...
          .some((r) =>
            String(r.msg ?? '').includes(
              'Arceus [query:vector]: VECTOR extension not supported on this platform',
            ),
          ),
      ).toBe(true);
    } finally {
