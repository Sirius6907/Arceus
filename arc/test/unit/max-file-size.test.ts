...
describe('getMaxFileSizeBytes', () => {
  const ORIGINAL = process.env.ARC_MAX_FILE_SIZE;
  ...
  afterEach(() => {
    if (ORIGINAL === undefined) {
      ...
      process.env.ARC_MAX_FILE_SIZE = ORIGINAL;
    }
    cap.restore();
  });
  ...
  it('clamps values above the tree-sitter ceiling', () => {
    ...
    process.env.ARC_MAX_FILE_SIZE = String(aboveCeilingKb);
  ...
  it.each(['abc', '0', '-512', '1.5', 'NaN', ''])(
    ...
    (raw) => {
      if (raw === '') {
        process.env.ARC_MAX_FILE_SIZE = raw;
        expect(getMaxFileSizeBytes()).toBe(DEFAULT_MAX_FILE_SIZE_BYTES);
        expect(cap.records().length).toBe(0);
        return;
      }
  ...
  it('warns separately for distinct invalid values', () => {
    process.env.ARC_MAX_FILE_SIZE = 'abc';
    getMaxFileSizeBytes();
    process.env.ARC_MAX_FILE_SIZE = 'xyz';
    getMaxFileSizeBytes();
...
describe('getMaxFileSizeBannerMessage', () => {
  ...
  it('returns null when the env var equals the default (in KB)', () => {
    process.env.ARC_MAX_FILE_SIZE = String(DEFAULT_MAX_FILE_SIZE_BYTES / 1024);
    expect(getMaxFileSizeBannerMessage()).toBeNull();
  });
