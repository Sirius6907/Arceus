...
describe('generateAIContextFiles', () => {
  ...
  beforeAll(async () => {
    ...
    storagePath = path.join(tmpDir, '.arc');
  ...
  it('creates or updates CLAUDE.md with Arceus section', async () => {
    const stats = { nodes: 50, edges: 100, processes: 5 };
    await generateAIContextFiles(tmpDir, storagePath, 'TestProject', stats);

    const claudeMdPath = path.join(tmpDir, 'CLAUDE.md');
    const content = await fs.readFile(claudeMdPath, 'utf-8');
    expect(content).toContain('arc:start');
    expect(content).toContain('arc:end');
  ...
  it('keeps the load-bearing repo-specific sections in the CLAUDE.md block (#856)', async () => {
    ...
    expect(content).toContain('If any Arceus tool warns the index is stale');
    expect(content).toContain('## Always Do');
    expect(content).toContain('## Never Do');
    expect(content).toContain('## Resources');
    expect(content).toContain('arc://repo/TestProject/context');
    expect(content).toContain('arc-impact-analysis/SKILL.md');
    expect(content).toContain('arc-refactoring/SKILL.md');
    expect(content).toContain('arc-debugging/SKILL.md');
    expect(content).toContain('arc-cli/SKILL.md');
  });
  ...
  it('keeps the CLAUDE.md Arceus block under the token-cost budget (#856)', async () => {
    ...
    const block = content.slice(
      content.indexOf('<!-- arc:start -->'),
      content.indexOf('<!-- arc:end -->'),
    );
  ...
  it('updates existing CLAUDE.md without duplicating', async () => {
    ...
    const starts = (content.match(/arc:start/g) || []).length;
  ...
  it('installs skills files', async () => {
    ...
    const skillsDir = path.join(tmpDir, '.claude', 'skills', 'arc');
    try {
  ...
  it('does not create .claude/skills/arc/ when skipSkills is true (#742)', async () => {
    ...
    // .claude/skills/arc/ from an earlier test.
    const skipDir = await fs.mkdtemp(path.join(os.tmpdir(), 'gn-ai-ctx-skip-skills-'));
    const skipStorage = path.join(skipDir, '.arc');
  ...
  it('writes nothing when both skipAgentsMd and skipSkills are true (--index-only, #742)', async () => {
    ...
    const idxStorage = path.join(idxDir, '.arc');
    ...
    try {
      ...
      await expect(fs.access(path.join(idxDir, '.claude', 'skills', 'arc'))).rejects.toThrow();
    } finally {
  ...
  it('omits standard skill references from AGENTS.md/CLAUDE.md when skipSkills is true (#742)', async () => {
    ...
    // .claude/skills/arc/*/SKILL.md files installed by installSkills().
    ...
    const noStdStorage = path.join(noStdDir, '.arc');
    ...
    try {
      ...
      expect(content).not.toContain('arc-exploring/SKILL.md');
      expect(content).not.toContain('arc-impact-analysis/SKILL.md');
      expect(content).not.toContain('arc-debugging/SKILL.md');
      expect(content).not.toContain('arc-refactoring/SKILL.md');
      expect(content).not.toContain('arc-guide/SKILL.md');
      expect(content).not.toContain('arc-cli/SKILL.md');
  ...
  it('preserves inline marker references in prose and does not corrupt markdown (#1041)', async () => {
    ...
    const bugStorage = path.join(bugDir, '.arc');
    ...
    try {
      ...
      expect((contentAfter.match(/<!-- arc:start -->/g) || []).length).toBe(2);
      expect((contentAfter.match(/<!-- arc:end -->/g) || []).length).toBe(2);
  ...
  it('matches section markers on files with CRLF line endings (#1041 cross-platform)', async () => {
    ...
    // `core.autocrlf`; when a section line ends `<!-- arc:start
    ...
    const crlfStorage = path.join(crlfDir, '.arc');
    ...
    const inlineProseLine =
      'See the `<!-- arc:start --> … <!-- arc:end -->` block in **[AGENTS.md](AGENTS.md)** for more.';
    const seeded = [
      '# Claude Code Rules',
      '',
      '## Arceus rules',
      ...
      '',
      '<!-- arc:start -->',
      '# Arceus — Code Intelligence (stale stub)',
      '<!-- arc:end -->',
      '',
    ].join('\r\n');
    ...
    try {
      ...
      expect((content.match(/<!-- arc:start -->/g) || []).length).toBe(2);
      expect((content.match(/<!-- arc:end -->/g) || []).length).toBe(2);
      ...
      expect(content).not.toContain('# Arceus — Code Intelligence (stale stub)');
    } finally {
