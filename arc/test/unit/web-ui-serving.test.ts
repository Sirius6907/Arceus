...
describe('landingPageHtml', () => {
  ...
  it('contains void background colour from arceus-web design tokens', () => {
    expect(html).toContain('#06060a');
  });

  it('contains surface card colour from arceus-web design tokens', () => {
    expect(html).toContain('#101018');
  });

  it('contains accent colour from arceus-web design tokens', () => {
  ...
  it('contains the build command in a terminal-style block', () => {
    expect(html).toContain('cd arceus-web');
...
describe('registerWebUI', () => {
  ...
  it('landing page handler returns styled HTML', async () => {
    ...
    expect(res.send).toHaveBeenCalledWith(expect.stringContaining('#f59e0b'));
  });
...
describe('resolveWebDistDir', () => {
  ...
  it('prefers ARC_WEB_DIST env var when set', async () => {
    const original = process.env.ARC_WEB_DIST;
    process.env.ARC_WEB_DIST = '/env/dist';
    try {
      accessMock.mockImplementation(async (p: string) => {
      ...
      if (original === undefined) {
        delete process.env.ARC_WEB_DIST;
      } else {
        process.env.ARC_WEB_DIST = original;
      }
    }
  });

  it('falls back to primary when ARC_WEB_DIST dir missing', async () => {
    const original = process.env.ARC_WEB_DIST;
    process.env.ARC_WEB_DIST = '/env/dist';
    try {
      accessMock.mockImplementation(async (p: string) => {
