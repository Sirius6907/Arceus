...
describe('setupOpenCode — JSONC preservation', () => {
  ...
  "model": "test"
    ...
    expect(config.mcp.arc).toBeDefined();
  ...
  it('updates existing arc MCP entry without losing other keys', async () => {
    execFileSyncMock.mockReturnValueOnce('/usr/local/bin/arc\n');
  ...
  "mcp": {
    ...
    expect(config.mcp.arc).toEqual({
      type: 'local',
      command: ['/usr/local/bin/arc', 'mcp'],
    });
  });
  ...
  it('does not wipe corrupt file content', async () => {
    ...
    expect(raw).not.toContain('arc');
  });

  it('uses npx fallback format when arc binary is not on PATH', async () => {
...
describe('setupCursor — JSONC preservation', () => {
  ...
  it('creates fresh mcp.json when missing', async () => {
    ...
    expect(config.mcpServers.arc).toBeDefined();
  });
...
describe('installClaudeCodeHooks — JSONC preservation', () => {
  ...
  "hooks": {
    ...
    expect(config.hooks.PreToolUse[1].hooks[0].command).toContain('arc-hook');
    expect(config.hooks.PostToolUse).toBeDefined();
  });

  it('does not add duplicate arc-hook entries', async () => {
          ...
          PreToolUse: [
            {
              ...
              hooks: [{ type: 'command', command: 'node "arc-hook.cjs"', timeout: 10 }],
            },
          ],
          PostToolUse: [
            {
