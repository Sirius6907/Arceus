...
// Static asset server for the arceus-web Docker image.
//
...
const server = createServer(async (req, res) => {
  ...
  try {
    ...
    res.writeHead(200, {
      'Cache-Control':
        finalPath.includes('/assets/') || finalPath.includes('\\assets\\')
...
server.listen(port, host, () => {
  console.log(`arceus-web listening on http://${host}:${port}`);
});
