...
| -------------- | -------------- | -------- | ------------------------------ |
| CLI + MCP core | `arc/`    | Vitest   | Primary test surface in CI     |
| Web UI         | `arceus-web/`| Vitest   | Unit/component tests           |
| Web UI E2E     | `arceus-web/`| Playwright | Run when changing UI flows   |
...
npm run test:coverage
npm run test:e2e            # Playwright (requires arc serve + npm run dev)
```
...
2. **`arceus-web/` files staged** → `tsc -b --noEmit`
3. **`arc/` files staged** → `tsc --noEmit`
...
- **Integration** — Real combinations (filesystem, MCP wiring, larger pipelines) as already organized under `arc/test/integration`.
...
- **E2E (web)** — Critical user paths only; prefer `data-testid` attributes for stable selectors. Tests run against real backend (`arc serve`) and Vite dev server.
...
| ------------------- | ---------------- | ------------------------------------------ |
| Unit coverage       | Align with CI    | CI runs Vitest with coverage in `arc` |
...
- **`ci-quality.yml`** — prettier format check, eslint lint, `tsc --noEmit` for `arc/`, `tsc -b --noEmit` for `arceus-web/`
- **`ci-tests.yml`** — `vitest run` with coverage (ubuntu) + cross-platform (macOS, Windows)
- **`ci-e2e.yml`** — Playwright E2E tests, gated on `arceus-web/**` changes

Local checks before pushing:

```bash
cd arc && npx tsc --noEmit && npm test
cd ../arceus-web && npx tsc -b --noEmit && npm test
```
