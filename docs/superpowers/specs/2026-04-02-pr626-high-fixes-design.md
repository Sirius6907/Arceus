...
**Date:** 2026-04-02
**PR:** Sirius6907/Arceus#626 — Intra-repo service communication tracking
**Scope:** 4 HIGH-priority issues identified by Sirius6907 and xkonjin
...
**File:** `arc/src/core/group/storage.ts`
...
- Sirius6907 explicitly requested `[a-zA-Z0-9_-]`
...
**File:** `arc/src/core/group/extractors/grpc-extractor.ts`
...
**File:** `arc/src/core/group/service-boundary-detector.ts`
...
**Files:**
- `arc/src/core/group/sync.ts` (lines 155-157) — per-id cleanup (KEEP)
- `arc/src/cli/group.ts` (line 188) — blanket `closeLbug()` (REMOVE)
