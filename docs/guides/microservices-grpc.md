# Using Arceus across gRPC microservices

## When to use this guide

This guide is for teams whose product lives in **several separate Git repositories** — one per service — and whose services talk to each other over **gRPC** (possibly alongside HTTP and message topics). Arceus indexes each repo independently, then a _group_ stitches the per-repo indexes into a single cross-repo view that the `impact`, `query`, and `context` tools can traverse. If your services live in one monorepo, much of this still applies — set each service as a member of a group and use the `service` prefix to scope queries — but the walkthrough assumes the harder multi-repo case.

## Mental model

- Each repository has its own `.arc/` index (a LadybugDB graph of symbols, relationships, processes). `arc analyze` in each repo produces that index completely independently.
- A **group** is a higher-level construct stored at `~/.arc/groups/<group>/` that references the per-repo indexes by their registry name.
...
- Contracts come from three places: automatic contract extractors (`grpc-extractor`, `http-route-extractor`, `topic-extractor`), a manifest escape hatch (`config.links` in `group.yaml`), and — for same-name symbol matches where no contract is declared — the exact-match matching cascade in [`matching.ts`](../../arc/src/core/group/matching.ts).
- Each repo stays editable and re-indexable on its own. Re-run `arc analyze` in a repo when it changes, then `arc group sync <group>` to refresh `contracts.json`. `arc group status` reports which members are stale.

## Prerequisites

- Arceus installed and runnable as `arc` or `npx arc` (see the root [README.md](../../README.md)).
...
- Write access to `~/.arc/` (the default arc home; see `getDefaultArceusDir` in [`storage.ts`](../../arc/src/core/group/storage.ts)).
...
Run `analyze` from inside each service repo (or pass the path). The CLI surface lives in [`arc/src/cli/analyze.ts`](../../arc/src/cli/analyze.ts) and is wired in [`arc/src/cli/index.ts`](../../arc/src/cli/index.ts).

```bash
cd ~/code/gateway && npx arc analyze
cd ~/code/orders  && npx arc analyze
cd ~/code/inventory && npx arc analyze
```
...
Each run writes a `.arc/` folder in the repo and registers the repo in `~/.arc/registry.json`. Confirm with `npx arc list`.

### 2. Author `group.yaml`

Create the group directory and edit the config. Either use the CLI scaffolder or write the file directly — both produce the same shape consumed by [`config-parser.ts`](../../arc/src/core/group/config-parser.ts).

```bash
npx arc group create payments-platform
# or manually:
mkdir -p ~/.arc/groups/payments-platform
$EDITOR   ~/.arc/groups/payments-platform/group.yaml
```
...
Field notes (schema in [`types.ts`](../../arc/src/core/group/types.ts)):

- `version` — must be `1`. The parser rejects anything else.
- `name` — required; used for the group directory name and all CLI / MCP calls.
- `repos` — a mapping from **group path** (a logical name you choose; can be a hierarchy like `backend/orders`) to **registry name** (the name shown by `npx arc list`). Both sides appear throughout the tooling: contract rows use the group path; `@<group>/<groupPath>` routes tools to a single member.
...
```bash
npx arc group sync payments-platform --verbose
```

What this does (see [`sync.ts`](../../arc/src/core/group/sync.ts)):

1. Opens each member's per-repo LadybugDB.
2. Runs the HTTP, gRPC, and topic extractors against the source files.
3. Applies manifest `links` through [`manifest-extractor.ts`](../../arc/src/core/group/extractors/manifest-extractor.ts).
...
The same operation is available over MCP as `group_sync({ name: "payments-platform" })` — see [`tools.ts`](../../arc/src/mcp/tools.ts).

### 4. Inspect the registry

Use `arc group contracts` for the CLI view or read the `arc://group/<name>/contracts` MCP resource for the same data.

```bash
npx arc group contracts payments-platform --type grpc --json
```
...
```bash
npx arc group impact payments-platform \
  --repo orders --target PlaceOrder --direction upstream --cross-depth 2
```

Phase 1 walks within the anchor member; Phase 2 hops across the Contract Bridge wherever a cross-link endpoint matches an impacted symbol. See [`cross-impact.ts`](../../arc/src/core/group/cross-impact.ts) for the bridge query.

## How gRPC extraction works

`GrpcExtractor` ([`grpc-extractor.ts`](../../arc/src/core/group/extractors/grpc-extractor.ts)) runs two passes per member repo:
...
2. **Source scan.** Every source file whose extension matches [`GRPC_SCAN_GLOB`](../../arc/src/core/group/extractors/grpc-patterns/index.ts) is parsed by its language plugin:

| Language | Provider signal | Consumer signal |
|----------|-----------------|-----------------|
| Go ([`go.ts`](../../arc/src/core/group/extractors/grpc-patterns/go.ts)) | `pb.RegisterXxxServer(...)`, `pb.UnimplementedXxxServer` embedded in struct | `pb.NewXxxClient(conn)` |
| Java ([`java.ts`](../../arc/src/core/group/extractors/grpc-patterns/java.ts)) | `extends XxxServiceGrpc.XxxServiceImplBase` (with or without `@GrpcService`) | `XxxServiceGrpc.newBlockingStub(...)`, `newStub(...)` |
| Python ([`python.ts`](../../arc/src/core/group/extractors/grpc-patterns/python.ts)) | `add_XxxServicer_to_server(...)` (bare or `_pb2_grpc.` attribute form) | `XxxStub(channel)` (ignores `Mock`/`Test`/`Fake`/`Stub`) |
| Node / TS ([`node.ts`](../../arc/src/core/group/extractors/grpc-patterns/node.ts)) | NestJS `@GrpcMethod('Service','Method')` | `@GrpcClient` field typed `XxxServiceClient`, `client.getService<X>('Service')`, `new XxxServiceClient(...)`, `new foo.bar.XxxService(...)` in files that call `loadPackageDefinition` |
...
The escape hatch is the `links` list in `group.yaml`, handled by [`ManifestExtractor`](../../arc/src/core/group/extractors/manifest-extractor.ts). Each entry is a **one-directional** provider/consumer declaration:
...
What the manifest extractor does (see [`manifest-extractor.ts`](../../arc/src/core/group/extractors/manifest-extractor.ts)):
...
History: the manifest extractor used to be silently skipped by the sync pipeline; that was fixed in [#827](https://github.com/Sirius6907/Arceus/pull/827) (tracking issue #826). If you ever see `config.links` with zero cross-links in `contracts.json`, make sure you're on a build that includes that fix, then re-run `group sync`.
...
2. **A known provider/consumer pair doesn't cross-link.** Most common cause: the package segment differs. Check the raw contract ids with `arc group contracts <name> --unmatched` — if you see two same-method contracts with different package prefixes, add a manifest `links:` entry to bridge them (no automatic rewrite rules yet).
...
5. **Cross-impact says "stale".** Both sides need a fresh per-repo index _and_ a fresh group sync. Order matters: `arc analyze` in each changed repo, then `arc group sync <name>`. Use `arc group status <name>` to see which side is behind.

## Related docs and references

- [AGENTS.md](../../AGENTS.md) — authoritative list of MCP tools and resources, including group-mode routing and the `arc://group/…` resources.
- [ARCHITECTURE.md](../../ARCHITECTURE.md) — overall data flow and the call-resolution DAG that the per-repo indexer uses.
- [`arc/src/core/group/`](../../arc/src/core/group/) — `service.ts`, `sync.ts`, `config-parser.ts`, `matching.ts`.
- [`arc/src/core/group/extractors/grpc-extractor.ts`](../../arc/src/core/group/extractors/grpc-extractor.ts) and [`grpc-patterns/`](../../arc/src/core/group/extractors/grpc-patterns/) — gRPC detection.
- [`arc/src/core/group/extractors/manifest-extractor.ts`](../../arc/src/core/group/extractors/manifest-extractor.ts) — the `config.links` escape hatch.
- [`arc/src/mcp/tools.ts`](../../arc/src/mcp/tools.ts) — MCP tool schemas (`group_list`, `group_sync`, plus `@<group>` routing on `impact` / `query` / `context`).
- [`arc/src/cli/group.ts`](../../arc/src/cli/group.ts) — CLI command definitions and flags.
- Upstream issues: [#701](https://github.com/Sirius6907/Arceus/issues/701), [#826](https://github.com/Sirius6907/Arceus/issues/826), [#906](https://github.com/Sirius6907/Arceus/issues/906).
