...
Arceus indexes COBOL codebases using a **regex-only extraction** strategy, bypassing tree-sitter entirely. This document explains why, how the pipeline works, and links to detailed sub-documents.
...
|----------|-------------|
| [File Detection](./file-detection.md) | Extension mapping, `ARC_COBOL_DIRS`, copybook classification |
...
| File | Purpose |
|------|---------|
| `arc/src/core/ingestion/cobol-preprocessor.ts` | Patch marker cleanup + regex extraction engine |
| `arc/src/core/ingestion/cobol-copy-expander.ts` | COPY statement expansion with REPLACING |
| `arc/src/core/ingestion/utils.ts` | `getLanguageFromPath`, `getLanguageFromFilename` |
| `arc/src/core/ingestion/pipeline.ts` | `isCobolCopybook`, `expandCobolCopies`, `detectCrossProgamContracts` |
| `arc/src/core/ingestion/workers/parse-worker.ts` | `processCobolRegexOnly` -- graph model builder |
| `arc/src/core/ingestion/workers/worker-pool.ts` | Configurable sub-batch size for COBOL |
