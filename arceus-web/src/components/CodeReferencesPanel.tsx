...
import { type GraphNode, getSyntaxLanguageFromFilename } from 'arceus-shared';
...
export const CodeReferencesPanel = ({ onFocusNode }: CodeReferencesPanelProps) => {
  ...
  const [panelWidth, setPanelWidth] = useState<number>(() => {
    try {
      const saved = window.localStorage.getItem('arc.codePanelWidth');
  ...
  useEffect(() => {
    try {
      window.localStorage.setItem('arc.codePanelWidth', String(panelWidth));
    } catch {
      // ignore
    }
  }, [panelWidth]);
