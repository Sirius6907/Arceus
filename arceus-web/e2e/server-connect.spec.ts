...
/**
 * E2E tests for the Arceus web UI — exploring view features.
 *
 * Requires:
 *   - arc serve running on localhost:4747 with at least one indexed repo
 *   - arceus-web dev server running on localhost:5173
 *
...
test.beforeAll(async () => {
  ...
  try {
    ...
    if (
      ...
      test.skip(true, 'arc serve not available on :4747');
      return;
    }
    if (
    ...
    if (backendRes.status === 'fulfilled') {
      ...
      if (!repos.length) {
        test.skip(true, 'No indexed repos — run arc analyze first');
        return;
      }
    }
  } catch {
...
test.describe('Arceus Ai', () => {
  test('panel opens and agent initializes without error', async ({ page }) => {
    await waitForGraphLoaded(page);

    await page.getByRole('button', { name: 'Arceus Ai' }).click();
