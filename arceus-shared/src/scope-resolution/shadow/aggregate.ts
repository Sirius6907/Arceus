 ...
 * reads `.arc/shadow-parity/latest.json` and renders.
 *
 * Related types — `ShadowAgreement`, `ShadowCallsite`, `ShadowDiff` — are
 * defined alongside `diffResolutions` in `./diff.ts` and re-exported
 * through the top-level `arceus-shared` barrel. Consumers import all
 * three from `arceus-shared`, not from this module.
 *
 ...
 *   `.arc/shadow-parity/latest.json` and diffing snapshots is useful).
