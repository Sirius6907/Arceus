"""
Arceus Docker Environment for SWE-bench Evaluation

Extends mini-swe-agent's Docker environment to:
1. Install Arceus (Node.js + npm + arc package)
2. Run `arc analyze` on the repository
...
Architecture:
  Agent bash cmd → /usr/local/bin/arc-query → curl localhost:4848/tool/query → eval-server → KuzuDB
  Fallback: → npx arc query (cold start, slower)
...
logger = logging.getLogger("arc_docker")

DEFAULT_CACHE_DIR = Path.home() / ".arc-eval-cache"
EVAL_SERVER_PORT = 4848


class ArceusDockerEnvironment(DockerEnvironment):
    """
    Docker environment with Arceus pre-installed, indexed, and eval-server running.

    Setup flow:
    1. Start Docker container (base SWE-bench image)
    2. Install Node.js + arc inside the container
    3. Run `arc analyze` (or restore from cache)
    4. Start `arc eval-server` daemon (keeps KuzuDB warm)
    5. Install standalone tool scripts in /usr/local/bin/
    6. Agent runs with near-instant Arceus tool calls
    """
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.arc_timeout = arc_timeout
        self.eval_server_port = eval_server_port
        self.index_time: float = 0.0
        self._arc_ready = False

    def start(self) -> dict:
        """Start the container and set up Arceus."""
        result = super().start()
        ...
        if self.enable_arc:
            try:
                ...
                log_safe_exception(
                    logger,
                    "Arceus setup failed, continuing without it",
                    e,
    ...
    def _setup_arc(self):
        """Install and configure Arceus in the container."""
        start = time.time()
        ...
        logger.info(f"Arceus setup completed in {self.index_time:.1f}s")
    ...
    def _index_repository(self):
        ...
        if cache_path.exists():
            logger.info(f"Restoring Arceus index from cache: {cache_key}")
            self._restore_cache(cache_path)
            return

        logger.info("Running arc analyze...")
        skip_flag = "--skip-embeddings" if self.skip_embeddings else ""
        result = self.execute({
            "command": f"cd /testbed && npx arc analyze . {skip_flag} 2>&1",
            "timeout": self.arc_timeout,
        })

        if result.get("returncode", 1) != 0:
            output = result.get("output", "")
            if "error" in output.lower() and "indexed" not in output.lower():
                raise RuntimeError(f"arc analyze failed: {output[-500:]}")

        self._save_cache(cache_path, repo_info)

    def _start_eval_server(self):
        """Start the Arceus eval-server daemon in the background."""
        logger.info(f"Starting eval-server on port {self.eval_server_port}...")

        self.execute({
            "command": (
                f"nohup npx arc eval-server --port {self.eval_server_port} "
                f"--idle-timeout 600 "
                f"> /tmp/arc-eval-server.log 2>&1 &"
            ),
            "timeout": 5,
        })
        ...
        log_output = self.execute({
            "command": "cat /tmp/arc-eval-server.log 2>/dev/null | tail -20",
        })
        logger.warning(
    ...
    def _render_tool_script(spec: ToolScriptSpec, port: str) -> str:
        """
        Render a standalone bash script for a Arceus tool.
        ...
        if spec.endpoint:
            lines.append(f'PORT="${{ARC_EVAL_PORT:-{port}}}"')
    ...
    def _install_tools(self):
        """
        Install standalone Arceus tool scripts in /usr/local/bin/.
        ...
        for spec in TOOL_SPECS.values():
            ...
            self.execute({
                "command": (
                    f"cat << 'ARC_SCRIPT_EOF' > /usr/local/bin/{spec.bin_name}\n"
                    f"{script_content}\n"
                    "ARC_SCRIPT_EOF\n"
        ...
        logger.info(f"Installed {len(TOOL_SPECS)} Arceus tool scripts in /usr/local/bin/")
    ...
    def _save_cache(self, cache_path: Path, repo_info: dict):
        """Save the Arceus index to the host cache directory."""
        try:
            cache_path.mkdir(parents=True, exist_ok=True)

            find_result = self.execute({
                "command": "find /root/.arc -name 'kuzu' -type d 2>/dev/null | head -1"
            })
            arc_dir = find_result.get("output", "").strip()

            if arc_dir:
                parent = str(Path(arc_dir).parent)
                self.execute({
                    "command": f"cd {parent} && tar czf /tmp/arc-cache.tar.gz .",
                    "timeout": 30,
                })
                ...
                if container_id:
                    ...
                    sp.run(
                        ["docker", "cp", f"{container_id}:/tmp/arc-cache.tar.gz",
                         str(cache_path / "index.tar.gz")],
                        check=True, capture_output=True,
                    )
                    (cache_path / "metadata.json").write_text(json.dumps(repo_info, indent=2))
                    logger.info(f"Cached Arceus index: {cache_path}")

        except Exception as e:
            log_safe_exception(
                logger,
                "Failed to cache Arceus index",
                e,
                include_debug=is_debug_enabled(),
                level="warning",
            )
    ...
    def _restore_cache(self, cache_path: Path):
        """Restore a cached Arceus index into the container."""
        try:
            ...
            if container_id:
                ...
                self.execute({"command": "mkdir -p /root/.arc"})

                storage_result = self.execute({
                    "command": "npx arc list 2>/dev/null | grep -o '/root/.arc/[^ ]*' | head -1 || echo '/root/.arc/repos/default'"
                })
                storage_path = storage_result.get("output", "").strip() or "/root/.arc/repos/default"
                self.execute({"command": f"mkdir -p {storage_path}"})

                sp.run(
                    ["docker", "cp", str(cache_tarball), f"{container_id}:/tmp/arc-cache.tar.gz"],
                    check=True, capture_output=True,
                )
                self.execute({
                    "command": f"cd {storage_path} && tar xzf /tmp/arc-cache.tar.gz",
                    "timeout": 30,
                })
                logger.info("Arceus index restored from cache")
    ...
    def get_template_vars(self) -> dict:
        """Add Arceus-specific template variables."""
        base_vars = super().get_template_vars()
        base_vars["arc_ready"] = self._arc_ready
        base_vars["arc_index_time"] = self.index_time
        return base_vars

    def serialize(self) -> dict:
        """Include Arceus environment info in serialization."""
        base = super().serialize()
        base.setdefault("info", {})["arc_env"] = {
            "enabled": self.enable_arc,
            "ready": self._arc_ready,
