"""Tests for MCPBridge._find_arc_command() and subprocess spawn."""
import subprocess
import unittest
from unittest.mock import MagicMock, call, patch


class TestFindArcCommand(unittest.TestCase):
    """Verify _find_arc_command() returns the correct command prefix."""
    ...
    def test_npx_path_returns_npx_arc(self):
        """When npx probe succeeds, command prefix is ['npx', 'arc']."""
        with patch("subprocess.run", return_value=self._success()) as mock_run:
            bridge = self._make_bridge()
            result = bridge._find_arc_command()

        self.assertEqual(result, ["npx", "arc"])
        mock_run.assert_called_once()
        args = mock_run.call_args[0][0]
        self.assertEqual(args, ["npx", "arc", "--version"])

    def test_global_path_returns_arc(self):
        """When npx probe fails but global install exists, prefix is ['arc']."""
...
class TestStartSpawnCommand(unittest.TestCase):
    ...
    def test_npx_path_spawns_npx_arc_mcp(self):
        """When npx path found, Popen must receive ['npx', 'arc', 'mcp']."""
        bridge = self._make_bridge()

        with patch.object(bridge, "_find_arc_command", return_value=["npx", "arc"]), \
        ...
        self.assertEqual(argv, ["npx", "arc", "mcp"])

    def test_global_path_spawns_arc_mcp(self):
        """When global path found, Popen must receive ['arc', 'mcp']."""
        bridge = self._make_bridge()

        with patch.object(bridge, "_find_arc_command", return_value=["arc"]), \
             patch("subprocess.Popen") as mock_popen, \
             patch.object(bridge, "_send_request", return_value={"protocolVersion": "2024-11-05"}), \
             patch.object(bridge, "_send_notification"):
    ...
    def test_arc_not_found_returns_false(self):
        """start() returns False and does not call Popen when arc not found."""
        bridge = self._make_bridge()

        with patch.object(bridge, "_find_arc_command", return_value=None), \
             patch("subprocess.Popen") as mock_popen:
