#!/usr/bin/env python3
"""
Results Analyzer for Arceus SWE-bench Evaluation
...
def arc_usage(
    ...
    """Analyze Arceus tool usage patterns across all runs."""
    results_path = Path(results_dir)
    runs = load_run_results(results_path)

    console.print("\n[bold]Arceus Tool Usage Analysis[/bold]\n")
    ...
    for run_id, run_data in sorted(runs.items()):
        ...
        for traj in run_data.get("trajectories", {}).values():
            gn = traj.get("info", {}).get("arc", {}).get("metrics", {})
