#!/usr/bin/env python3
"""
Arceus SWE-bench Evaluation Runner

Main entry point for running SWE-bench evaluations with and without Arceus.
Supports running a single configuration or a full matrix of models x modes.

Usage:
    # Single run (default: native_augment mode — Arceus tools + grep enrichment)
...
logger = logging.getLogger("arc_eval")
console = Console()
...
def _build_environment(config: dict, instance: dict):
    ...
    if env_class_name == "eval.environments.arc_docker.ArceusDockerEnvironment":
        from environments.arc_docker import ArceusDockerEnvironment

        env_config["image"] = get_swebench_docker_image(instance)
        return ArceusDockerEnvironment(**env_config)
...
def _build_agent(config: dict, model, env, instance_dir: Path, instance_id: str):
    """Construct the Arceus agent with trajectory output configured."""
    from agents.arc_agent import ArceusAgent

    agent_config = dict(config.get("agent", {}))
    agent_config.pop("agent_class", "eval.agents.arc_agent.ArceusAgent")
    traj_path = instance_dir / f"{instance_id}.traj.json"
    agent_config["output_path"] = traj_path
    return ArceusAgent(model, env, **agent_config)
...
def process_instance(
    ...
    try:
        ...
        result["arc_metrics"] = agent.arc_metrics.to_dict()
...
def list_configs():
    ...
    for name in AVAILABLE_MODES:
        ...
        gn_mode = config.get("agent", {}).get("arc_mode", "baseline")
        console.print(f"  {name:<20} arc_mode={gn_mode}")
...
def _print_summary(results: list[dict], model: str, mode: str):
    ...
    # Arceus-specific metrics
    gn_tool_calls = sum(
        r.get("arc_metrics", {}).get("total_tool_calls", 0) for r in results
    )
    gn_augment_hits = sum(
        r.get("arc_metrics", {}).get("augmentation_hits", 0) for r in results
    )
    if gn_tool_calls > 0:
        table.add_row("Arceus Tool Calls", str(gn_tool_calls))
...
def _print_matrix_summary(all_results: dict[str, list[dict]]):
    ...
    for run_id, results in sorted(all_results.items()):
        ...
        gn_calls = sum(r.get("arc_metrics", {}).get("total_tool_calls", 0) for r in results)
