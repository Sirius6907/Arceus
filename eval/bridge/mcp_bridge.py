"""
MCP Bridge for Arceus

Starts the Arceus MCP server as a subprocess and provides a Python interface
...
from constants import (
    MCP_FIND_ARC_FALLBACK_TIMEOUT_SECONDS,
    MCP_FIND_ARC_TIMEOUT_SECONDS,
...
class MCPBridge:
    """
    Manages a Arceus MCP server subprocess and proxies tool calls to it.
    ...
    def start(self) -> bool:
        """Start the Arceus MCP server subprocess."""
        if self._started:
            return True
        ...
        try:
            ...
            init_result = self._send_request("initialize", {
                ...
                "capabilities": {},
                "clientInfo": {"name": "arc-eval", "version": "0.1.0"},
            })
    ...
    def call_tool(self, tool_name: str, arguments: dict[str, Any] | None = None) -> dict[str, Any] | None:
        """
        Call a Arceus MCP tool and return the result.
    ...
    def _find_arc_command(self) -> list[str] | None:
        """Find the arc CLI command prefix."""
        ...
        try:
            result = subprocess.run(
                ...
                capture_output=True,
                text=True,
                timeout=MCP_FIND_ARC_TIMEOUT_SECONDS,
                cwd=self.repo_path,
            )
        ...
        try:
            result = subprocess.run(
                ...
                capture_output=True,
                text=True,
                timeout=MCP_FIND_ARC_FALLBACK_TIMEOUT_SECONDS,
            )
            if result.returncode == 0:
                return ["arc"]
        except Exception:
...
class MCPToolCLI:
    ...
    def run(self, tool_name: str, args_json: str = "{}") -> int:
        ...
        if not self.bridge.start():
            print("ERROR: Failed to start Arceus MCP bridge", file=sys.stderr)
            return 1
