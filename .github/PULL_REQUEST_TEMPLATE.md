...
- [ ] `arc/` (CLI / core / MCP server)
- [ ] `arceus-web/` (Vite / React UI)
...
- [ ] `cd arc && npm test`
- [ ] `cd arc && npm run test:integration` *(if core/indexing/MCP paths changed)*
- [ ] `cd arc && npx tsc --noEmit`
- [ ] `cd arceus-web && npm test` *(if web changed)*
- [ ] `cd arceus-web && npx tsc -b --noEmit` *(if web changed)*
- [ ] Manual / Playwright E2E *(note environment — see `arceus-web/e2e/`)*

## Risk & rollout

<!-- Breaking changes, migrations, index refresh (`npx arc analyze`), release notes -->
