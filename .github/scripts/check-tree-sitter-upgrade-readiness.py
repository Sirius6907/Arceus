...
def _load_package_json() -> dict:
    return json.loads((ARC_DIR / "package.json").read_text())
...
def read_pinned_grammar_versions() -> dict[str, str]:
    """Return the grammar version range pinned in arc/package.json.
...
def vendored_drift_summary(
    ...
    """Inspect a vendored grammar under arc/vendor/<name>.
    ...
    """
    vendor_dir = ARC_DIR / "vendor" / name
    pkg: dict = {}
...
def main() -> int:
    ...
    for name, (upstream_repo, upstream_branch, parser_path) in sorted(GRAMMARS.items()):
        ...
        # we ship our own copy under arc/vendor/<name>. Treat them
        ...
        installed_parser = ARC_DIR / "node_modules" / name / parser_path
        if not installed_parser.is_file():
            # Fallback to default location.
            installed_parser = ARC_DIR / "node_modules" / name / "src" / "parser.c"
    ...
    if vendored_grammars:
        ...
        lines.append(
            "These grammars ship from `arc/vendor/` rather than the npm "
