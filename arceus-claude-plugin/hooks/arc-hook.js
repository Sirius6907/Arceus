#!/usr/bin/env node
/**
 * Arceus Claude Code Plugin Hook
 *
 * PreToolUse  — intercepts Grep/Glob/Bash searches and augments
 *               with graph context from the Arceus index.
...
/**
 * Find the .arc directory by walking up from startDir.
 * Returns the path to .arc/ or null if not found.
 */
...
/**
 * Walk up from `startDir` looking for a non-registry `.arc/` folder.
 * Returns the path to `.arc/` or null if not found within 5 levels.
 */
function walkForArceusDir(startDir) {
  let dir = startDir;
  for (let i = 0; i < 5; i++) {
    const candidate = path.join(dir, '.arc');
 ...
 * `git worktree add`. Linked worktrees never contain `.arc/`, so the
...
function findArceusDir(startDir) {
  const cwd = startDir || process.cwd();

  // Fast path: the cwd is inside the canonical repo (most common case).
  const fromCwd = walkForArceusDir(cwd);
  if (fromCwd) return fromCwd;

  // Fallback: cwd may be inside a linked git worktree whose `.arc/`
  ...
  if (canonicalRoot && canonicalRoot !== cwd) {
    return walkForArceusDir(canonicalRoot);
  }
  return null;
}
...
/**
 * Spawn a arc CLI command synchronously.
 * Detects binary on PATH once, then runs exactly once.
 *
 * SECURITY: Never use shell: true with user-controlled arguments.
 * On Windows, invoke arc.cmd directly (no shell needed).
 */
function runArceusCli(args, cwd, timeout) {
  const isWin = process.platform === 'win32';

  // Detect whether 'arc' is on PATH (cheap check, no execution)
  let useDirectBinary = false;
  try {
    const which = spawnSync(isWin ? 'where' : 'which', ['arc'], {
      encoding: 'utf-8',
      timeout: 3000,
      stdio: ['pipe', 'pipe', 'pipe'],
    });
  ...
  if (useDirectBinary) {
    return spawnSync(isWin ? 'arc.cmd' : 'arc', args, {
      encoding: 'utf-8',
      timeout,
      cwd,
      stdio: ['pipe', 'pipe', 'pipe'],
  ...
  return spawnSync(isWin ? 'npx.cmd' : 'npx', ['-y', 'arc', ...args], {
    encoding: 'utf-8',
...
function handlePreToolUse(input) {
  ...
  const arceusDir = findArceusDir(cwd);
  if (!arceusDir) return;
  ...
  const release = acquireHookSlot(arceusDir);
  if (!release) return;

  let result = '';
  try {
    const child = runArceusCli(['augment', '--', pattern], cwd, 7000);
 ...
 *
 * Instead of spawning a full `arc analyze` synchronously (which blocks
 * the agent for up to 120s and risks LadybugDB corruption on timeout), we do a
 * lightweight staleness check: compare `git rev-parse HEAD` against the
 * lastCommit stored in `.arc/meta.json`. If they differ, notify the
...
function handlePostToolUse(input) {
  ...
  try {
    const meta = JSON.parse(fs.readFileSync(path.join(arceusDir, 'meta.json'), 'utf-8'));
  ...
  const analyzeCmd = `npx arc analyze${hadEmbeddings ? ' --embeddings' : ''}`;
  sendHookResponse(
    'PostToolUse',
    `Arceus index is stale (last indexed: ${lastCommit ? lastCommit.slice(0, 7) : 'never'}). ` +
...
function main() {
  try {
    ...
    if (process.env.ARC_DEBUG) {
      console.error('Arceus hook error:', (err.message || '').slice(0, 200));
    }
  }
}
