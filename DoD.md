# Definition of Done — Arceus

Last reviewed: 2026-04-23 · Version: 2.0.0

This document defines the repo-wide completion bar for production-ready changes in Arceus. It is the stable baseline. Implementation prompts, agent behavior, and review workflows may add task-specific checks, but they must never weaken this bar.
...
- CLI, MCP, and HTTP-bridge behavior in `arc/`
- Browser UI in `arceus-web/`
- Shared contracts in `arceus-shared/`
...
- [ ] The change is placed in the correct package and layer:
  - `arc/` for CLI, MCP, HTTP bridge, ingestion, graph, and runtime logic
  - `arceus-web/` for browser UI (thin client — no WASM workers, all queries via HTTP API)
  - `arceus-shared/` for shared contracts, types, and constants
- [ ] Pipeline and architecture boundaries remain explicit. Shared ingestion code in `arc/src/core/ingestion/` must not name languages — use `LanguageProvider` hooks (see `AGENTS.md` and `ARCHITECTURE.md` § Call-Resolution DAG).
...
- [ ] Direct imports from `arceus-shared` are used. No barrel re-exports introduced to paper over drift between packages.
...
- [ ] Existing contracts (types in `arceus-shared/`, CLI flags, MCP tools/resources, HTTP routes, graph node/edge shapes, persisted IDs) are preserved unless the task explicitly requires a contract change.
...
- [ ] **Impact analysis.** Where Arceus graph tooling is available and relevant, impact of non-trivial symbol, contract, or runtime-path changes is checked **before** editing.
...
- [ ] `arceus-shared/` dist is built before consuming packages are typechecked or tested (CI uses the `setup-arc` action for this — local runs must match).

### 4.2 If `arc/` changed

- [ ] `cd arc && npx tsc --noEmit`
- [ ] `cd arc && npm test`
- [ ] `cd arc && npx prettier --check .` for files in the diff (pre-commit runs the affected-tests subset; do not expand scope)

### 4.3 If `arceus-web/` changed

- [ ] `cd arceus-web && npx tsc -b --noEmit`
- [ ] `cd arceus-web && npm test`
- [ ] `cd arceus-web && npm run test:e2e` when browser flows or user-facing UI behavior changed

### 4.4 If `arceus-shared/` changed

- [ ] Shared package builds cleanly (`npm run build` in `arceus-shared/`)
...
- [ ] The workflow passes a dry-run or triggered run before merge; concurrency (`cancel-in-progress`) and the `setup-arc` action remain wired correctly.
...
- A contract drifted between `arc/`, `arceus-web/`, and `arceus-shared/` and only one side was updated.
