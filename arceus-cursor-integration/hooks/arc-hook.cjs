#!/usr/bin/env node
/**
 * Arceus Cursor postToolUse Hook
 *
 ...
 * command), runs `arc augment <pattern>`, and emits the enriched context
 ...
 * string value in tool_input. Set ARC_DEBUG=1 to log the raw payload
...
function resolveCliPath() {
  try {
    return require.resolve('arc/dist/cli/index.js');
  } catch {
    return '';
  }
}
...
function main() {
  try {
    ...
    if (process.env.ARC_DEBUG) {
      console.error('Arceus Cursor hook error:', (err.message || '').slice(0, 200));
    }
  }
}
